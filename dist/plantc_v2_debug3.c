...
PlantArray* parse_action_decl(PlantArray* tokens, long pos) {
  ...
  if (strcmp(arrow_lx, "->") == 0) {
    ...
    fprintf(stderr, "DEBUG p5=%ld count=%ld after_tok=%p\n", p5, tokens->count, (void*)after_tok); if(after_tok) fprintf(stderr, "DEBUG tok0=[%s] tok1=[%s]\n", plant_list_get(after_tok,0), plant_list_get(after_tok,1));
