...
PlantArray* parse_action_decl(PlantArray* tokens, long pos) {
  ...
  if (strcmp(arrow_lx, "->") == 0) {
    { long ti; fprintf(stderr, "DEBUG TOKENS:"); for(ti=0;ti<tokens->count;ti++){tx_t tt=tok_type(plant_list_get(tokens,ti)); tx_t tv=tok_lex(plant_list_get(tokens,ti)); fprintf(stderr, " [%ld]%s=%s",ti,tt,tv); } fprintf(stderr, "\n"); }
