...
tx_t make_node(tx_t ty, tx_t val, PlantArray* kids) {
  return plant_list_make(6,"type",ty,"value",val,"children",kids);
}

tx_t make_leaf(tx_t ty, tx_t val) {
  return make_node(ty,val,plant_list_make(0));
}

tx_t make_unary(tx_t ty, tx_t op, tx_t operand) {
  return make_node(ty,op,plant_list_make(1,operand));
}

tx_t make_binary(tx_t ty, tx_t op, tx_t left, tx_t right) {
  return make_node(ty,op,plant_list_make(2,left,right));
}
...
tx_t match_ident_or_keyword(tx_t src, long i, long n) {
  ...
  return plant_list_make(2,wd,_from_long(ni));
}
...
tx_t match_number(tx_t src, long i, long n) {
  ...
  return plant_list_make(2,num,_from_long(ni));
}
...
tx_t match_string(tx_t src, long i, long n) {
  ...
  return plant_list_make(2,val,_from_long(si));
}
...
tx_t scan_tokens(tx_t src) {
  ...
  while (i<n) {
    ...
    if (strcmp(ch, "*") == 0 && i+1<n && strcmp(char_at(src,i+1), "*") == 0) {
      tokens = plant_list_push(tokens, plant_list_make(2,"STAR_STAR","**"));
    }
    if (strcmp(ch, "*") == 0 && i+1<n && strcmp(char_at(src,i+1), "*") == 0) {
      i = i+2;
    }
...
tx_t consume(PlantArray* tokens, long pos) {
  return plant_list_make(2,plant_list_get(tokens, pos),_from_long(pos+1));
}
...
long _second(PlantArray* pair) {
  return _to_long(plant_list_get(pair, 1));
}
...
tx_t collect_value(PlantArray* tokens, long start) {
  ...
  while (1) {
    ...
    if (is_eof_flag) {
      return plant_list_make(2,text,_from_long(p2));
    }
    tok = peek(tokens, p2);
    lx = tok_lex(tok);
    tt = tok_type(tok);
...
tx_t parse_create_stmt(PlantArray* tokens, long pos) {
  ...
  if (strcmp(lx2, "=") == 0) {
    ...
    p5 = _second(vpair);
    return plant_list_make(2,plant_list_make(8,"type","create_stmt","target",id_name,"var_type",vtype,"value",expr),_from_long(p5));
  }
  if (strcmp(lx2, "TO") == 0) {
  ...
  expr = plant_list_get(vpair, 0);
  p4 = _second(vpair);
  return plant_list_make(2,plant_list_make(8,"type","create_stmt","target",id_name,"var_type",vtype,"value",expr),_from_long(p4));
}
...
tx_t parse_show_stmt(PlantArray* tokens, long pos) {
  ...
  expr = plant_list_get(vpair, 0);
  p3 = _second(vpair);
  return plant_list_make(2,plant_list_make(4,"type","show_stmt","value",expr),_from_long(p3));
}
...
tx_t parse_give_stmt(PlantArray* tokens, long pos) {
  ...
  expr = plant_list_get(vpair, 0);
  p3 = _second(vpair);
  return plant_list_make(2,plant_list_make(4,"type","give_stmt","value",expr),_from_long(p3));
}
...
tx_t parse_set_stmt(PlantArray* tokens, long pos) {
  ...
  expr = plant_list_get(vpair, 0);
  p5 = _second(vpair);
  return plant_list_make(2,plant_list_make(6,"type","set_stmt","target",id_name,"value",expr),_from_long(p5));
}
...
tx_t parse_let_stmt(PlantArray* tokens, long pos) {
  ...
  if (strcmp(lx2, "=") == 0) {
    ...
    expr = plant_list_get(vpair, 0);
    p5 = _second(vpair);
    return plant_list_make(2,plant_list_make(8,"type","let_stmt","target",id_name,"var_type",vtype,"value",expr),_from_long(p5));
  }
  if (strcmp(lx2, "TO") == 0) {
    to_pair = consume(tokens, p3);
    p4 = _second(to_pair);
  ...
  expr = plant_list_get(vpair, 0);
  p4 = _second(vpair);
  return plant_list_make(2,plant_list_make(8,"type","let_stmt","target",id_name,"var_type",vtype,"value",expr),_from_long(p4));
}
...
tx_t parse_reap_stmt(PlantArray* tokens, long pos) {
  ...
  while (1) {
    ...
    if (is_eof_flag) {
      return plant_list_make(2,plant_list_make(8,"type","reap_stmt","target",var_name,"action",act_name,"args",args),_from_long(p5));
    }
    tok = peek(tokens, p5);
    lx = tok_lex(tok);
    if (strcmp(lx, ".") == 0) {
      ...
      p6 = _second(dot);
      return plant_list_make(2,plant_list_make(8,"type","reap_stmt","target",var_name,"action",act_name,"args",args),_from_long(p6));
    }
    arg_text = "";
    adepth = 0;
    while (1) {
...
tx_t parse_put_stmt(PlantArray* tokens, long pos) {
  ...
  p6 = _second(dot_pair);
  return plant_list_make(2,plant_list_make(6,"type","put_stmt","item",item,"target",target),_from_long(p6));
}
...
tx_t parse_break_stmt(PlantArray* tokens, long pos) {
  ...
  p3 = _second(dot_pair);
  return plant_list_make(2,plant_list_make(2,"type","break_stmt"),_from_long(p3));
}
...
tx_t parse_continue_stmt(PlantArray* tokens, long pos) {
  ...
  dot_pair = consume(tokens, p2);
  p3 = _second(dot_pair);
  return plant_list_make(2,plant_list_make(2,"type","continue_stmt"),_from_long(p3));
}
...
tx_t parse_if_stmt(PlantArray* tokens, long pos) {
  ...
  p4 = _second(com);
  body = plant_list_make(0);
  while (1) {
    is_eof_flag = is_eof(tokens, p4);
    if (is_eof_flag) {
      return plant_list_make(2,plant_list_make(6,"type","if_stmt","cond",cond,"body",body),_from_long(p4));
    }
    tok = peek(tokens, p4);
    lx = tok_lex(tok);
    if (strcmp(lx, "/") == 0) {
      ...
      p7 = _second(dot);
      return plant_list_make(2,plant_list_make(6,"type","if_stmt","cond",cond,"body",body),_from_long(p7));
    }
...
tx_t parse_season_stmt(PlantArray* tokens, long pos) {
  ...
  while (1) {
    is_eof_flag = is_eof(tokens, p4);
    if (is_eof_flag) {
      return plant_list_make(2,plant_list_make(6,"type","season_stmt","cond",cond,"body",body),_from_long(p4));
    }
    tok = peek(tokens, p4);
    lx = tok_lex(tok);
    if (strcmp(lx, "/") == 0) {
      ...
      dot = consume(tokens, p6);
      p7 = _second(dot);
      return plant_list_make(2,plant_list_make(6,"type","season_stmt","cond",cond,"body",body),_from_long(p7));
    }
    stmt_pair = parse_statement(tokens, p4);
    stmt = plant_list_get(stmt_pair, 0);
    p4 = _second(stmt_pair);
...
tx_t parse_statement(PlantArray* tokens, long pos) {
  ...
  return plant_list_make(2,NULL,_from_long(pos+1));
}
...
tx_t parse_enum_decl(PlantArray* tokens, long pos) {
  ...
  p4 = _second(lbr);
  members = plant_list_make(0);
  while (1) {
    is_eof_flag = is_eof(tokens, p4);
    if (is_eof_flag) {
      return plant_list_make(2,plant_list_make(6,"type","enum_decl","name",name,"members",members),_from_long(p4));
    }
    tok = peek(tokens, p4);
    lx = tok_lex(tok);
    if (strcmp(lx, "}") == 0) {
      ...
      p5 = _second(rbr);
      return plant_list_make(2,plant_list_make(6,"type","enum_decl","name",name,"members",members),_from_long(p5));
    }
...
tx_t parse_action_decl(PlantArray* tokens, long pos) {
  ...
  lp = consume(tokens, p3);
  p4 = _second(lp);
  params = plant_list_make(0);
  while (1) {
    is_eof_flag = is_eof(tokens, p4);
    if (is_eof_flag) {
      return plant_list_make(2,plant_list_make(8,"type","action_decl","name",aname,"params",params,"body",plant_list_make(0)),_from_long(p4));
    }
    tok = peek(tokens, p4);
    lx = tok_lex(tok);
    if (strcmp(lx, ")") == 0) {
  ...
  if (strcmp(arrow_lx, "->") == 0) {
    ...
    if (strcmp(after_lx, ".") == 0) {
      dot_pair = consume(tokens, p5);
      p5 = _second(dot_pair);
      return plant_list_make(2,plant_list_make(8,"type","action_decl","name",aname,"params",params,"body",plant_list_make(0)),_from_long(p5));
    }
  }
  body = plant_list_make(0);
  while (1) {
    is_eof_flag = is_eof(tokens, p5);
    if (is_eof_flag) {
      return plant_list_make(2,plant_list_make(8,"type","action_decl","name",aname,"params",params,"body",body),_from_long(p5));
    }
    tok4 = peek(tokens, p5);
    lx4 = tok_lex(tok4);
    if (strcmp(lx4, "/") == 0) {
      ...
      p8 = _second(dot);
      return plant_list_make(2,plant_list_make(8,"type","action_decl","name",aname,"params",params,"body",body),_from_long(p8));
    }
...
tx_t _handle_cat(tx_t expr) {
  ...
  i = 1;
  while (i<(parts)->count) {
    pi = plant_list_get(parts, i);
    res = _cat(_cat(_cat(res, ", "), pi), ")");
    i = i+1;
  }
  return res;
}
...
tx_t generate_node(tx_t node, long indent) {
  ...
  if (strcmp(ntype, "action_decl") == 0) {
    ...
    ccode = _cat(_cat(_cat(_cat("tx_t ", aname), "("), paramstr), ") \{\n");
