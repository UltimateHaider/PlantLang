...
PlantArray* parse_reap_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t act_name = "";
  long p5 = 0;
  PlantArray* next_tok = NULL;
  tx_t next_lx = "";
  tx_t next_ty = "";
  PlantArray* colon_pair = NULL;
  PlantArray* func_pair = NULL;
  tx_t func_name = "";
...
PlantArray* parse_if_stmt(PlantArray* tokens, long pos) {
  ...
  while (1) {
    ...
    if (is_eof_flag) {
      return plant_list_make((int64_t)(2), plant_list_make((int64_t)(6), "type", "if_stmt", "cond", cond, "body", body), p4);
    }
    tok = peek(tokens, p4);
    lx = tok_lex(tok);
    if (strcmp(lx, "/") == 0) {
      ...
      p7 = _second(dot);
      return plant_list_make((int64_t)(2), plant_list_make((int64_t)(6), "type", "if_stmt", "cond", cond, "body", body), p7);
    }
...
PlantArray* parse_action_decl(PlantArray* tokens, long pos) {
  ...
  PlantArray* skip_tok = NULL;
  tx_t skip_ty = "";
  PlantArray* skip_pair = NULL;
...
tx_t _handle_cat(tx_t expr) {
  ...
  res = _cat(res, plant_list_get(parts, 0));
  i = 1;
  while (i<(parts)->count) {
    res = _cat(_cat(_cat(res, ", "), plant_list_get(parts, i)), ")");
    i = i+1;
  }
  return res;
}
...
tx_t generate_node(tx_t node, long indent) {
  ...
  if (strcmp(ntype, "show_stmt") == 0) { fprintf(stderr, "SHOW_VAL: [%s]\n", val); cval = translate_expr(val); fprintf(stderr, "SHOW_XL: [%s]\n", cval); cval = _handle_cat(cval); fprintf(stderr, "SHOW_HC: [%s]\n", cval); isel = indent_str(indent); return _cat(_cat(_cat(isel, "  plant_print("), cval), ");\n"); }
  ...
  if (strcmp(ntype, "reap_stmt") == 0) {
    ...
    astr = _handle_cat(argstr);
    isel = indent_str(indent);
    if (strcmp(tgt, "_") == 0) {
      return _cat(_cat(_cat(_cat(_cat(isel, "  "), act), "("), astr), ");\n");
    }
    if (strcmp(tgt, "_") != 0) {
      return _cat(_cat(_cat(_cat(_cat(_cat(_cat(isel, "  "), tgt), " = "), act), "("), astr), ");\n");
    }
  }
