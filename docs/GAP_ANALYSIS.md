...
2. **List/map/std library surface**: array & map literals, slices, `FIRST/LAST/SUM/AVERAGE/MEDIAN/UNIQUE/REVERSE/FLATTEN/CHUNK/ZIP/RANGE`, string `FIND/COUNT_OF/JOIN/SLICE` (v0.48.26 shipped `UPPER/LOWER/TRIM/INCLUDES/STARTS_WITH/ENDS_WITH/REVERSE/REPEAT/PAD/PAD_LEFT` via the `strings:` FFI module), `math` extras (`LOG PI E SIGN CLAMP`), `fs:APPEND`.
