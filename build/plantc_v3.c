...
tx_t _handle_func_paren3(tx_t expr, tx_t kw, tx_t cfn);
tx_t _handle_func_paren4(tx_t expr, tx_t kw, tx_t cfn);
tx_t _handle_func_paren5(tx_t expr, tx_t kw, tx_t cfn);
...
tx_t _union_init(tx_t expr, PlantArray* nums, PlantArray* evars);
...
tx_t gen_set_stmt(tx_t node, PlantArray* nums, PlantArray* evars, tx_t isel);
tx_t gen_give_stmt(tx_t node, PlantArray* nums, PlantArray* evars, tx_t rty, tx_t actx, tx_t mexit, tx_t isel);
tx_t gen_if_stmt(tx_t node, PlantArray* nums, PlantArray* evars, tx_t isel, long indent_num, PlantArray* env);
tx_t gen_reap_stmt(tx_t node, PlantArray* sigs, PlantArray* subst, PlantArray* clmap, PlantArray* nums, PlantArray* stvars, PlantArray* evars, tx_t isel);
tx_t gen_season_stmt(tx_t node, PlantArray* nums, PlantArray* evars, tx_t isel, long indent_num, PlantArray* env);
tx_t gen_cycle_stmt(tx_t node, PlantArray* nums, PlantArray* evars, tx_t isel, long indent_num, PlantArray* env);
tx_t gen_weather_stmt(tx_t node, tx_t isel, long indent_num, PlantArray* env);
tx_t gen_throw_stmt(tx_t node, PlantArray* nums, PlantArray* evars, tx_t isel);
...
tx_t generate_node(tx_t node, PlantArray* env);
...
tx_t is_union_type(tx_t t, PlantArray* reg);
tx_t collect_union_names(PlantArray* ast);
...
tx_t _map_replace(PlantArray* m, tx_t k, tx_t v);
tx_t _ta_collect_list(PlantArray* lst, PlantArray* tsub);
tx_t _ta_rw_list(PlantArray* lst, PlantArray* tsub);
tx_t _ts_resolve(PlantArray* tsub, tx_t name);
...
tx_t is_keyword(tx_t wrd) {
    ...
    if (strcmp(wrd,"SETUP") == 0) {
    return 1;
    }
    if (strcmp(wrd,"TEARDOWN") == 0) {
    return 1;
    }
    if (strcmp(wrd,"NOT") == 0) {
    return 1;
...
tx_t keyword_to_type(tx_t wrd) {
    ...
    if (strcmp(wrd,"ASINH") == 0) {
    return "ASINH";
    }
    if (strcmp(wrd,"ACOSH") == 0) {
    return "ACOSH";
    }
    if (strcmp(wrd,"ATANH") == 0) {
    return "ATANH";
    }
    if (strcmp(wrd,"ERF") == 0) {
    return "ERF";
    }
    if (strcmp(wrd,"ERFC") == 0) {
    return "ERFC";
    }
    if (strcmp(wrd,"GAMMA") == 0) {
    return "GAMMA";
    }
    if (strcmp(wrd,"LGAMMA") == 0) {
    return "LGAMMA";
    }
    if (strcmp(wrd,"EXP2") == 0) {
    return "EXP2";
    }
    if (strcmp(wrd,"LOG_BASE") == 0) {
    return "LOG_BASE";
    }
    if (strcmp(wrd,"ENUM") == 0) {
    return "ENUM";
...
tx_t scan_tokens(tx_t src) {
  ...
  tx_t tok_ty = "";
    ...
    if (strcmp(ch,"'") == 0) {
    sqi = match_string(src, i, n);
    }
    if (strcmp(ch,"'") == 0) {
            tokens = plant_list_add(tokens, plant_list_make ( 3 , "STRING" , plant_list_get(sqi ,  0 ) , _from_long ( ls0 ) ));
    }
    if (strcmp(ch,"'") == 0) {
    i = plant_list_get(sqi ,  1 );
    }
    if (strcmp(ch,"'") == 0) {
                      continue;
    }
...
tx_t parse_field_access(PlantArray* tokens, long pos, tx_t text, tx_t dpth) {
  ...
  tx_t cut = "";
    lexer = get_lexer();
    ftok = plant_iLexer_peek_at(lexer, tokens, pos+1);
    flx = plant_iLexer_tok_lex(lexer, ftok);
    fty = plant_iLexer_tok_type(lexer, ftok);
    ...
    nt2_ty = plant_iLexer_tok_type(lexer, nt2);
    ...
    ltk = plant_iLexer_peek_at(lexer, tokens, pos - 1);
    lt_ty = plant_iLexer_tok_type(lexer, ltk);
    if (strcmp(dpth,"0") != 0) {
    if (strcmp(lt_ty,"IDENT") != 0) {
    return plant_list_make ( 3 , "0" , text , pos );
    }
    }
    dotp = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(dotp);
    fldp = plant_iLexer_consume_at(lexer, tokens, p2);
    fld = plant_iLexer_tok_lex(lexer, plant_list_get(fldp ,  0 ));
    p3 = _second(fldp);
    tx_t nfld = _cat3("\"", fld, "\"");
    tx_t fin = "_map_get(";
    ...
    if (strcmp(lt_ty,"IDENT") == 0) {
    if (strcmp(ttail,"\")") != 0) {
    lt_lx = plant_iLexer_tok_lex(lexer, ltk);
...
tx_t parse_method_call(PlantArray* tokens, long pos, tx_t text) {
  ...
  tx_t mcp4 = "";
    lexer = get_lexer();
    mtok0 = plant_iLexer_peek_at(lexer, tokens, pos+1);
    mname = plant_iLexer_tok_lex(lexer, mtok0);
    ltk0 = plant_iLexer_peek_at(lexer, tokens, pos - 1);
    lt_ty0 = plant_iLexer_tok_type(lexer, ltk0);
    ttail0 = substring(text, strlen( text ) - 2, strlen( text ));
    tx_t recv = text;
    tx_t pre = "";
    if (strcmp(lt_ty0,"IDENT") == 0) {
    if (strcmp(ttail0,"\")") != 0) {
    lt_lx0 = plant_iLexer_tok_lex(lexer, ltk0);
    recv = lt_lx0;
    ...
    mcp = plant_iLexer_consume_at(lexer, tokens, pos);
    mpp = _second(mcp);
    mcp2 = plant_iLexer_consume_at(lexer, tokens, mpp);
    mpp = _second(mcp2);
    mcp3 = plant_iLexer_consume_at(lexer, tokens, mpp);
    mpp = _second(mcp3);
    mvp = collect_value(tokens, mpp);
    margs = _first(mvp);
    mpp = _second(mvp);
    mcp4 = plant_iLexer_consume_at(lexer, tokens, mpp);
    mpp = _second(mcp4);
    tx_t minner = mfn;
    minner = _cat(minner, "(");
...
tx_t collect_value(PlantArray* tokens, long start) {
  ...
  tx_t cpair = "";
    ...
    long depth = 0;
    while (1) {
    is_eof_flag = plant_iLexer_is_eof_at(lexer, tokens, p2);
    if (is_eof_flag) {
    return plant_list_make ( 2 , text , p2 );
    }
    tok = plant_iLexer_peek_at(lexer, tokens, p2);
    lx = plant_iLexer_tok_lex(lexer, tok);
    tt = plant_iLexer_tok_type(lexer, tok);
    ...
    cpair = plant_iLexer_consume_at(lexer, tokens, p2);
    p2 = _second(cpair);
    return plant_list_make ( 2 , text , p2 );
    }
    if (strcmp(lx,")") == 0 && depth == 0) {
...
tx_t collect_args(PlantArray* tokens, long pos) {
  ...
  tx_t com2 = "";
    ...
    long p5 = pos;
    while (1) {
    is_eof_flag = plant_iLexer_is_eof_at(lexer, tokens, p5);
    if (is_eof_flag) {
    return plant_list_make ( 2 , args , p5 );
    }
    tok = plant_iLexer_peek_at(lexer, tokens, p5);
    lx = plant_iLexer_tok_lex(lexer, tok);
    ty = plant_iLexer_tok_type(lexer, tok);
    if (strcmp(lx,",") == 0 && strcmp(ty,"STRING") != 0) {
    com = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(com);
    }
    if (strcmp(lx,".") == 0 && strcmp(ty,"STRING") != 0) {
    return plant_list_make ( 2 , args , p5 );
    ...
    long adepth = 0;
    while (1) {
    atok = plant_iLexer_peek_at(lexer, tokens, p5);
    alx = plant_iLexer_tok_lex(lexer, atok);
    atype = plant_iLexer_tok_type(lexer, atok);
    ...
    cp = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(cp);
    }
        args = plant_list_add(args, arg_text);
    tok2 = plant_iLexer_peek_at(lexer, tokens, p5);
    lx2 = plant_iLexer_tok_lex(lexer, tok2);
    if (strcmp(lx2,",") == 0) {
    com2 = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(com2);
    }
    }
  return collect_args;
...
tx_t parse_await_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p7 = "";
    ...
    p2 = _second(pair);
    act_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    act_name = plant_iLexer_tok_lex(lexer, plant_list_get(act_pair ,  0 ));
    p3 = _second(act_pair);
    ...
    lx = plant_iLexer_tok_lex(lexer, tok);
    if (strcmp(lx,"IN") == 0) {
    in_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(in_pair);
    ctx_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    ctx_name = plant_iLexer_tok_lex(lexer, plant_list_get(ctx_pair ,  0 ));
    p6 = _second(ctx_pair);
    dot = plant_iLexer_consume_at(lexer, tokens, p6);
    p7 = _second(dot);
    return plant_list_make ( 2 , plant_list_make ( 8 , "type" , "await_stmt" , "action" , act_name , "args" , args , "ctx" , ctx_name ) , p7 );
    }
    dot = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(dot);
...
tx_t parse_async_in_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p8 = "";
    ...
    pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(pair);
    in_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    p3 = _second(in_pair);
    ctx_pair = plant_iLexer_consume_at(lexer, tokens, p3);
    ctx_name = plant_iLexer_tok_lex(lexer, plant_list_get(ctx_pair ,  0 ));
    p4 = _second(ctx_pair);
    com = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(com);
    act_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    act_name = plant_iLexer_tok_lex(lexer, plant_list_get(act_pair ,  0 ));
    p6 = _second(act_pair);
    ap = collect_args(tokens, p6);
    PlantArray* args = plant_list_get(ap ,  0 );
    p7 = _second(ap);
    dot = plant_iLexer_consume_at(lexer, tokens, p7);
    p8 = _second(dot);
...
tx_t parse_cancel_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p6 = "";
    ...
    lx = plant_iLexer_tok_lex(lexer, tok);
    if (strcmp(lx,"IN") == 0) {
    in_pair = plant_iLexer_consume_at(lexer, tokens, p3);
    p4 = _second(in_pair);
    ctx_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    ctx_name = plant_iLexer_tok_lex(lexer, plant_list_get(ctx_pair ,  0 ));
    p5 = _second(ctx_pair);
    dot = plant_iLexer_consume_at(lexer, tokens, p5);
    p6 = _second(dot);
...
tx_t parse_trace_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p7 = "";
    ...
    pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(pair);
    lv_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    lv = plant_iLexer_tok_lex(lexer, plant_list_get(lv_pair ,  0 ));
    p3 = _second(lv_pair);
    vpair = collect_value(tokens, p3);
    tx_t expr = plant_list_get(vpair ,  0 );
    p4b = _second(vpair);
    tx_t ctx_name = "";
    tok = plant_iLexer_peek_at(lexer, tokens, p4b);
    lx = plant_iLexer_tok_lex(lexer, tok);
    if (strcmp(lx,"IN") == 0) {
    in_pair = plant_iLexer_consume_at(lexer, tokens, p4b);
    p5 = _second(in_pair);
    ctx_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    ctx_name = plant_iLexer_tok_lex(lexer, plant_list_get(ctx_pair ,  0 ));
    p6 = _second(ctx_pair);
...
tx_t parse_mission_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p6 = "";
    ...
    pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(pair);
    cfg_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    p3 = _second(cfg_pair);
    key_pair = plant_iLexer_consume_at(lexer, tokens, p3);
    cfg_key = plant_iLexer_tok_lex(lexer, plant_list_get(key_pair ,  0 ));
    p4 = _second(key_pair);
    eq_tok = plant_iLexer_peek_at(lexer, tokens, p4);
    eq_lx = plant_iLexer_tok_lex(lexer, eq_tok);
    if (strcmp(eq_lx,"=") == 0 || strcmp(eq_lx,"IS") == 0) {
    eq_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(eq_pair);
    }
    val_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    cfg_val = plant_iLexer_tok_lex(lexer, plant_list_get(val_pair ,  0 ));
    vty = plant_iLexer_tok_type(lexer, plant_list_get(val_pair ,  0 ));
...
tx_t parse_create_stmt(PlantArray* tokens, long pos, PlantArray* rtab, tx_t emode) {
  ...
  tx_t to_pair = "";
    ...
    p4 = _second(eq_pair);
    cb_tok = plant_iLexer_peek_at(lexer, tokens, p4);
    cb_lx = plant_iLexer_tok_lex(lexer, cb_tok);
    if (strcmp(cb_lx,"[") == 0) {
    ...
    dotp = plant_iLexer_consume_at(lexer, tokens, p5);
    p6 = _second(dotp);
...
tx_t parse_array_decl(PlantArray* tokens, long pos, PlantArray* rtab) {
  ...
  tx_t p9 = "";
    lexer = get_lexer();
    arr_pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(arr_pair);
    lb_tok = plant_iLexer_peek_at(lexer, tokens, p2);
    lb_lx = plant_iLexer_tok_lex(lexer, lb_tok);
    if (strcmp(lb_lx,"[") != 0) {
    return plant_list_make ( 2 , plant_list_make ( 6 , "type" , "syntax_error" , "message" , "expected bracket after array" ) , p2 );
    }
    lb_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    p3 = _second(lb_pair);
    et_pair = collect_type_text(tokens, p3, "]", 1);
    elem_type = _first(et_pair);
    p4 = _second(et_pair);
    comma_tok = plant_iLexer_peek_at(lexer, tokens, p4);
    comma_lx = plant_iLexer_tok_lex(lexer, comma_tok);
    if (strcmp(comma_lx,",") == 0) {
    comma_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(comma_pair);
    }
    sz_pair = collect_type_text(tokens, p4, "]", 0);
    arr_size = _first(sz_pair);
    p5 = _second(sz_pair);
    rb_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p6 = _second(rb_pair);
    tx_t _lb = _cat ( "[" , "" );
    tx_t _rb = _cat ( "]" , "" );
    tx_t _cm = ", ";
    tx_t vtype = _cat ( _cat4 ( "ARRAY" , _lb , elem_type , _cm ) , _cat ( arr_size , _rb ) );
    name_pair = plant_iLexer_consume_at(lexer, tokens, p6);
    id_name = plant_iLexer_tok_lex(lexer, plant_list_get(name_pair ,  0 ));
    p7 = _second(name_pair);
    tok2 = plant_iLexer_peek_at(lexer, tokens, p7);
    lx2 = plant_iLexer_tok_lex(lexer, tok2);
    long p8 = p7;
    if (strcmp(lx2,"=") == 0) {
    eq_pair = plant_iLexer_consume_at(lexer, tokens, p7);
    p8 = _second(eq_pair);
    }
    if (strcmp(lx2,"TO") == 0) {
    to_pair = plant_iLexer_consume_at(lexer, tokens, p7);
    p8 = _second(to_pair);
    }
    vpair = collect_value(tokens, p8);
    expr = _first(vpair);
    p9 = _second(vpair);
    return plant_list_make ( 2 , plant_list_make ( 8 , "type" , "create_stmt" , "target" , id_name , "var_type" , vtype , "value" , expr ) , p9 );
}
tx_t parse_show_stmt(PlantArray* tokens, long pos) {
  tx_t lexer = "";
  tx_t pair = "";
...
tx_t parse_verify_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t sp = "";
    ...
    pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(pair);
    lbl_tok = plant_iLexer_peek_at(lexer, tokens, p2);
    lbl_ty = plant_iLexer_tok_type(lexer, lbl_tok);
    if (strcmp(lbl_ty,"STRING") == 0) {
    lbl_lx = plant_iLexer_tok_lex(lexer, lbl_tok);
    lbl_lx = escape_string(lbl_lx);
    lbl_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    p3 = _second(lbl_pair);
    lbl = _cat("\"", lbl_lx);
    lbl = _cat(lbl, "\"");
    }
    ...
    cond = _first(vp);
    p4 = _second(vp);
    sep_tok = plant_iLexer_peek_at(lexer, tokens, p4);
    sep_ty = plant_iLexer_tok_type(lexer, sep_tok);
    if (strcmp(sep_ty,"PERIOD") == 0) {
    sp = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(sp);
    }
...
tx_t parse_setup_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t sp = "";
    ...
    val = _first(vp);
    p3 = _second(vp);
    sep_tok = plant_iLexer_peek_at(lexer, tokens, p3);
    sep_ty = plant_iLexer_tok_type(lexer, sep_tok);
    if (strcmp(sep_ty,"PERIOD") == 0) {
    sp = plant_iLexer_consume_at(lexer, tokens, p3);
    p3 = _second(sp);
    }
...
tx_t parse_suite_stmt(PlantArray* tokens, long pos, tx_t clv, PlantArray* ctab, PlantArray* rtab, long bstart, tx_t emode) {
  ...
  tx_t stmt = "";
    ...
    ntok = plant_iLexer_peek_at(lexer, tokens, p2);
    nty = plant_iLexer_tok_type(lexer, ntok);
    if (strcmp(nty,"STRING") == 0) {
    nl = plant_iLexer_tok_lex(lexer, ntok);
    nl = escape_string(nl);
    ncons = plant_iLexer_consume_at(lexer, tokens, p2);
    p2 = _second(ncons);
    sname = nl;
    ctok = plant_iLexer_peek_at(lexer, tokens, p2);
    clx = plant_iLexer_tok_lex(lexer, ctok);
    if (strcmp(clx,",") == 0) {
    ccons = plant_iLexer_consume_at(lexer, tokens, p2);
    p2 = _second(ccons);
    }
    }
    while (1) {
    ...
    tt = plant_iLexer_tok_type(lexer, tok);
    if (strcmp(lx,"/") == 0) {
    slash = plant_iLexer_consume_at(lexer, tokens, p2);
    p3 = _second(slash);
    kw = plant_iLexer_consume_at(lexer, tokens, p3);
    p4 = _second(kw);
    dot = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(dot);
    return plant_list_make ( 2 , plant_list_make ( 6 , "type" , "suite_stmt" , "name" , sname , "body" , body ) , _from_long ( p5 ) );
    }
    if (strcmp(tt,"SETUP") == 0) {
    sb_pair = parse_setup_stmt(tokens, p2);
    sb_node = _first(sb_pair);
            body = plant_list_add(body, sb_node);
    p2 = _second(sb_pair);
                      continue;
    }
    if (strcmp(tt,"TEARDOWN") == 0) {
    td_pair = parse_teardown_stmt(tokens, p2);
    td_node = _first(td_pair);
            body = plant_list_add(body, td_node);
    p2 = _second(td_pair);
                      continue;
    }
    stmt_pair = parse_statement(tokens, p2, clv, ctab, rtab, bstart, emode);
    stmt = _first(stmt_pair);
    if (strcmp(stmt,"") > 0) {
...
tx_t parse_now_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p3 = "";
    ...
    tx_t fmt = "";
    ntok = plant_iLexer_peek_at(lexer, tokens, p2);
    nlx = plant_iLexer_tok_lex(lexer, ntok);
    if (strcmp(nlx,"FORMAT") == 0) {
    fpair = plant_iLexer_consume_at(lexer, tokens, p2);
    p2 = _second(fpair);
    ctok = plant_iLexer_peek_at(lexer, tokens, p2);
    cty = plant_iLexer_tok_type(lexer, ctok);
    ...
    cpair = plant_iLexer_consume_at(lexer, tokens, p2);
    p2 = _second(cpair);
    qpair = plant_iLexer_consume_at(lexer, tokens, p2);
    qlx = plant_iLexer_tok_lex(lexer, plant_list_get(qpair ,  0 ));
    qty = plant_iLexer_tok_type(lexer, plant_list_get(qpair ,  0 ));
    p2 = _second(qpair);
    ...
    dpair = plant_iLexer_consume_at(lexer, tokens, p2);
    dlx = plant_iLexer_tok_lex(lexer, plant_list_get(dpair ,  0 ));
    p3 = _second(dpair);
    if (strcmp(dlx,".") != 0) {
...
tx_t parse_free_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p4 = "";
    ...
    id_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    id_name = plant_iLexer_tok_lex(lexer, plant_list_get(id_pair ,  0 ));
    id_type = plant_iLexer_tok_type(lexer, plant_list_get(id_pair ,  0 ));
    p3 = _second(id_pair);
    ...
    dot_pair = plant_iLexer_consume_at(lexer, tokens, p3);
    p4 = _second(dot_pair);
...
tx_t parse_wait_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p4 = "";
    ...
    p3 = _second(mpair);
    if (strcmp(ms,"") > 0) {
    mtok = plant_iLexer_peek_at(lexer, tokens, p2);
    mty = plant_iLexer_tok_type(lexer, mtok);
...
tx_t parse_arc_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t f_pair = "";
    ...
    pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(pair);
    k_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    k_lx = plant_iLexer_tok_lex(lexer, plant_list_get(k_pair ,  0 ));
    p3 = _second(k_pair);
    a_pair = plant_iLexer_consume_at(lexer, tokens, p3);
    a_name = plant_iLexer_tok_lex(lexer, plant_list_get(a_pair ,  0 ));
    p4 = _second(a_pair);
    if (strcmp(k_lx,"LINK") == 0) {
    t_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(t_pair);
    c_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    c_name = plant_iLexer_tok_lex(lexer, plant_list_get(c_pair ,  0 ));
    p6 = _second(c_pair);
    dot_pair = plant_iLexer_consume_at(lexer, tokens, p6);
    p7 = _second(dot_pair);
    return plant_list_make ( 2 , plant_list_make ( 6 , "type" , "arc_link_stmt" , "parent" , a_name , "child" , c_name ) , p7 );
    }
    f_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(f_pair);
    c_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    c_name = plant_iLexer_tok_lex(lexer, plant_list_get(c_pair ,  0 ));
    p6 = _second(c_pair);
...
tx_t parse_fast_reset_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p4 = "";
    ...
    pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(pair);
    r_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    r_lx = plant_iLexer_tok_lex(lexer, plant_list_get(r_pair ,  0 ));
    p3 = _second(r_pair);
...
tx_t _lower_dots(tx_t e) {
  ...
  tx_t idv = "";
    ...
    if (strcmp(ch,".") == 0 && i > 0 && i + 1 < n) {
    pv = char_at(e, i - 1);
    nx = char_at(e, i+1);
    pid = _is_idch(pv);
    nid0 = _is_idch(nx);
    nz = str_eq(nx, "_");
    if (strcmp(pid,"1") == 0 && ( strcmp(nid0,"1") == 0 || strcmp(nz,"1") == 0 )) {
    long js = i - 1;
    while (js > 0) {
    pj = char_at(e, js - 1);
    pjok = _is_idch(pj);
    ...
    base = substring(e, js, i);
    PlantArray* ids = plant_list_make ( 0 );
    tx_t pending = base;
    long k = i+1;
    while (k < n) {
    kid = char_at(e, k);
    ...
    k = k+1;
                    ...
                    ids = plant_list_add(ids, w);
    dchk = char_at(e, k);
    disdot = str_eq(dchk, ".");
    if (strcmp(disdot,"1") == 0) {
    k = k+1;
    }
    if (strcmp(disdot,"1") != 0) {
                                              break;
    ...
    long mi = 0;
    while (mi < plant_array_length(ids)) {
    idv = plant_list_get(ids, mi);
    res = "_map_get(";
    res = _cat(res, pending);
    res = _cat(res, ", \"");
    res = _cat(res, idv);
    res = _cat(res, "\")");
    pending = res;
...
tx_t _swap_super_prefix(PlantArray* bd, tx_t parent) {
  ...
  tx_t nnx = "";
    ...
    if (strcmp(vx,"") > 0) {
    nvx = strings_REPLACE(vx, "SUPER_", _cat(parent, "_"));
...
tx_t parse_give_stmt(PlantArray* tokens, long pos, tx_t clv) {
  ...
  tx_t lexpr = "";
    ...
    tx_t expr = plant_list_get(vpair ,  0 );
    p3 = _second(vpair);
    gtok = plant_iLexer_peek_at(lexer, tokens, p3);
    glx = plant_iLexer_tok_lex(lexer, gtok);
    if (strcmp(glx,"AS") == 0) {
    gtok2 = plant_iLexer_peek_at(lexer, tokens, p3+1);
    glx2 = plant_iLexer_tok_lex(lexer, gtok2);
    if (strcmp(glx2,"RESPONSE") == 0) {
    as_pair = plant_iLexer_consume_at(lexer, tokens, p3);
    p4 = _second(as_pair);
    rs_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(rs_pair);
    tx_t rjson = "";
    rtok = plant_iLexer_peek_at(lexer, tokens, p5);
    rlx = plant_iLexer_tok_lex(lexer, rtok);
    if (strcmp(rlx,"JSON") == 0) {
    drop_rj = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(drop_rj);
    rjson = "1";
    }
    dot_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p6 = _second(dot_pair);
    return plant_list_make ( 2 , plant_list_make ( 8 , "type" , "respond_stmt" , "content" , expr , "req" , clv , "json" , rjson ) , p6 );
    }
    }
    lexpr = _lower_dots(expr);
    expr = lexpr;
...
tx_t parse_incdec_stmt(PlantArray* tokens, long pos, tx_t op) {
  ...
  tx_t p5 = "";
    ...
    id_name = plant_iLexer_tok_lex(lexer, plant_list_get(id_pair ,  0 ));
    p3 = _second(id_pair);
    by_pair = plant_iLexer_consume_at(lexer, tokens, p3);
    by_lx = plant_iLexer_tok_lex(lexer, plant_list_get(by_pair ,  0 ));
    p4 = _second(by_pair);
...
tx_t parse_bloom_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t dtp = "";
    lexer = get_lexer();
    kw_pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p = _second(kw_pair);
    spair = plant_iLexer_consume_at(lexer, tokens, p);
    species = plant_iLexer_tok_lex(lexer, plant_list_get(spair ,  0 ));
    p = _second(spair);
    aspair = plant_iLexer_consume_at(lexer, tokens, p);
    aslx = plant_iLexer_tok_lex(lexer, plant_list_get(aspair ,  0 ));
    if (strcmp(aslx,"AS") != 0) {
    tx_t berr = "BLOOM requires AS var";
    return plant_list_make ( 2 , plant_list_make ( 4 , "type" , "syntax_error" , "msg" , berr ) , p );
    }
    p = _second(aspair);
    vpair = plant_iLexer_consume_at(lexer, tokens, p);
    vty = plant_iLexer_tok_type(lexer, plant_list_get(vpair ,  0 ));
    ...
    var = plant_iLexer_tok_lex(lexer, plant_list_get(vpair ,  0 ));
    p = _second(vpair);
    dtp = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(dtp);
...
tx_t _parse_ds_pattern(PlantArray* tokens, long pos) {
  ...
  tx_t cmpair = "";
    lexer = get_lexer();
    op_pair = plant_iLexer_consume_at(lexer, tokens, pos);
    oty = plant_iLexer_tok_type(lexer, plant_list_get(op_pair ,  0 ));
    p = _second(op_pair);
    tx_t kind = "array";
    tx_t closek = "RBRACKET";
    if (strcmp(oty,"LBRACE") == 0) {
    kind = "object";
    closek = "RBRACE";
    }
    ...
    kp = plant_iLexer_consume_at(lexer, tokens, p);
    key = plant_iLexer_tok_lex(lexer, plant_list_get(kp ,  0 ));
    p = _second(kp);
    nm = key;
    sub = "";
    ctok = plant_iLexer_peek_at(lexer, tokens, p);
    clx = plant_iLexer_tok_lex(lexer, ctok);
    if (strcmp(clx,":") == 0) {
    cp_pair = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(cp_pair);
    ntok = plant_iLexer_peek_at(lexer, tokens, p);
    nty = plant_iLexer_tok_type(lexer, ntok);
    ...
    if (strcmp(nty,"LBRACE") != 0 && strcmp(nty,"LBRACKET") != 0) {
    npair = plant_iLexer_consume_at(lexer, tokens, p);
    bn = plant_iLexer_tok_lex(lexer, plant_list_get(npair ,  0 ));
    p = _second(npair);
    nm = bn;
    }
    }
    ...
    if (strcmp(kind,"object") != 0) {
    etok = plant_iLexer_peek_at(lexer, tokens, p);
    ety = plant_iLexer_tok_type(lexer, etok);
    ...
    if (strcmp(ety,"LBRACE") != 0 && strcmp(ety,"LBRACKET") != 0) {
    npair = plant_iLexer_consume_at(lexer, tokens, p);
    en = plant_iLexer_tok_lex(lexer, plant_list_get(npair ,  0 ));
    p = _second(npair);
                items = plant_list_add(items, plant_list_make ( 6 , "key" , "" , "name" , en , "sub" , "" ));
    }
    }
    stok = plant_iLexer_peek_at(lexer, tokens, p);
    slx = plant_iLexer_tok_lex(lexer, stok);
    if (strcmp(slx,",") == 0) {
    cmpair = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(cmpair);
    }
    }
...
tx_t parse_let_stmt(PlantArray* tokens, long pos, PlantArray* rtab, tx_t emode) {
  ...
  tx_t to_pair = "";
    ...
    pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(pair);
    dtk = plant_iLexer_peek_at(lexer, tokens, p2);
    dty = plant_iLexer_tok_type(lexer, dtk);
    ...
    eqtok = plant_iLexer_peek_at(lexer, tokens, p3d);
    eqlx = plant_iLexer_tok_lex(lexer, eqtok);
    if (strcmp(eqlx,"=") != 0) {
    tx_t dderr = "destructuring requires = expression";
    return plant_list_make ( 2 , plant_list_make ( 4 , "type" , "syntax_error" , "msg" , dderr ) , p3d );
    }
    eqpair = plant_iLexer_consume_at(lexer, tokens, p3d);
    p4d = _second(eqpair);
...
tx_t parse_closure(PlantArray* tokens, long pos, PlantArray* rtab, tx_t emode) {
  ...
  tx_t ep = "";
    lexer = get_lexer();
    long start_pos = pos;
    lb = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(lb);
    ...
    ty = plant_iLexer_tok_type(lexer, tok);
    if (strcmp(lx,"]") == 0) {
    rb = plant_iLexer_consume_at(lexer, tokens, p2);
    p2 = _second(rb);
                      break;
    }
    if (strcmp(lx,"(") == 0) {
    ...
    ent_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    en0 = plant_iLexer_tok_lex(lexer, plant_list_get(ent_pair ,  0 ));
    p2 = _second(ent_pair);
    tx_t en_mode = "";
    tx_t en_name = "";
    ...
    npair = plant_iLexer_consume_at(lexer, tokens, p2);
    en_name = plant_iLexer_tok_lex(lexer, plant_list_get(npair ,  0 ));
    p2 = _second(npair);
    }
    if (strcmp(en_mode,"") == 0) {
    ...
    tok2 = plant_iLexer_peek_at(lexer, tokens, p2);
    lx2 = plant_iLexer_tok_lex(lexer, tok2);
    if (strcmp(lx2,",") == 0) {
    had_comma = "1";
    com = plant_iLexer_consume_at(lexer, tokens, p2);
    p2 = _second(com);
    }
    }
    ptok = plant_iLexer_peek_at(lexer, tokens, p2);
    plx = plant_iLexer_tok_lex(lexer, ptok);
    ...
    lp = plant_iLexer_consume_at(lexer, tokens, p2);
    p3 = _second(lp);
    while (1) {
    is_eof_flag = plant_iLexer_is_eof_at(lexer, tokens, p3);
    if (is_eof_flag) {
    return plant_list_make ( 2 , plant_list_make ( 0 ) , start_pos );
    }
    tok = plant_iLexer_peek_at(lexer, tokens, p3);
    lx = plant_iLexer_tok_lex(lexer, tok);
    if (strcmp(lx,")") == 0) {
    rp = plant_iLexer_consume_at(lexer, tokens, p3);
    p4 = _second(rp);
                              break;
    }
    pn_pair = plant_iLexer_consume_at(lexer, tokens, p3);
    pn = plant_iLexer_tok_lex(lexer, plant_list_get(pn_pair ,  0 ));
    p4 = _second(pn_pair);
    tok2 = plant_iLexer_peek_at(lexer, tokens, p4);
    lx2 = plant_iLexer_tok_lex(lexer, tok2);
    if (strcmp(lx2,"(") == 0) {
    lp2 = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(lp2);
    ptv = collect_type_text(tokens, p5, ")", 1);
    pt = _first(ptv);
    p5 = _second(ptv);
    rp2 = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(rp2);
    ...
    tok3 = plant_iLexer_peek_at(lexer, tokens, p4);
    lx3 = plant_iLexer_tok_lex(lexer, tok3);
    if (strcmp(lx3,",") == 0) {
    com2 = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(com2);
    }
    p3 = p4;
    }
    ...
    atok = plant_iLexer_peek_at(lexer, tokens, p4);
    alx = plant_iLexer_tok_lex(lexer, atok);
    if (strcmp(alx,"->") != 0) {
    return plant_list_make ( 2 , plant_list_make ( 0 ) , start_pos );
    }
    ap = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(ap);
    stok = plant_iLexer_peek_at(lexer, tokens, p5);
    sty = plant_iLexer_tok_type(lexer, stok);
    if (strcmp(sty,"MINUS") == 0) {
    sp = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(sp);
    }
    tx_t body_kind = "expr";
    btok = plant_iLexer_peek_at(lexer, tokens, p5);
    blx = plant_iLexer_tok_lex(lexer, btok);
    PlantArray* body = plant_list_make ( 0 );
    if (strcmp(blx,"(") == 0) {
    btok2 = plant_iLexer_peek_at(lexer, tokens, p5+1);
    blx2 = plant_iLexer_tok_lex(lexer, btok2);
    ...
    long bstart7 = p6;
    bp = plant_iLexer_consume_at(lexer, tokens, p5);
    p6 = _second(bp);
    PlantArray* stmts = plant_list_make ( 0 );
    while (1) {
    is_eof_flag = plant_iLexer_is_eof_at(lexer, tokens, p6);
    if (is_eof_flag) {
                                      break;
    }
    btok3 = plant_iLexer_peek_at(lexer, tokens, p6);
    blx3 = plant_iLexer_tok_lex(lexer, btok3);
    if (strcmp(blx3,")") == 0) {
    brp = plant_iLexer_consume_at(lexer, tokens, p6);
    p6 = _second(brp);
                                      break;
    }
    ...
    etok = plant_iLexer_peek_at(lexer, tokens, p5);
    elx = plant_iLexer_tok_lex(lexer, etok);
    ety = plant_iLexer_tok_type(lexer, etok);
    ...
    text = _cat(text, elx);
    ep = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(ep);
    }
    body = text;
    }
...
tx_t parse_reap_stmt(PlantArray* tokens, long pos, PlantArray* rtab, tx_t emode) {
  ...
  tx_t com = "";
    ...
    pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(pair);
    var_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    var_name = plant_iLexer_tok_lex(lexer, plant_list_get(var_pair ,  0 ));
    p3 = _second(var_pair);
    from_pair = plant_iLexer_consume_at(lexer, tokens, p3);
    p4 = _second(from_pair);
    act_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    act_name = plant_iLexer_tok_lex(lexer, plant_list_get(act_pair ,  0 ));
    act_ty = plant_iLexer_tok_type(lexer, plant_list_get(act_pair ,  0 ));
    p5 = _second(act_pair);
    next_tok = plant_iLexer_peek_at(lexer, tokens, p5);
    next_lx = plant_iLexer_tok_lex(lexer, next_tok);
    next_ty = plant_iLexer_tok_type(lexer, next_tok);
    if (strcmp(next_ty,"COLON") == 0) {
    act_name = _cat(act_name, ":");
    colon_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(colon_pair);
    func_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    func_name = plant_iLexer_tok_lex(lexer, plant_list_get(func_pair ,  0 ));
    act_name = _cat(act_name, func_name);
    p5 = _second(func_pair);
    }
    tx_t is_genargs = "0";
    ga_tok = plant_iLexer_peek_at(lexer, tokens, p5);
    ga_lx = plant_iLexer_tok_lex(lexer, ga_tok);
    if (strcmp(ga_lx,"[") == 0) {
    ga_lb = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(ga_lb);
    ...
    if (isgt == 1) {
    p5 = _second(gv);
    ga_rb = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(ga_rb);
    ...
    ex_tok = plant_iLexer_peek_at(lexer, tokens, p5);
    ex_lx = plant_iLexer_tok_lex(lexer, ex_tok);
    ex_ty = plant_iLexer_tok_type(lexer, ex_tok);
    ...
    tx_t has_cparen = "0";
    cp_tok = plant_iLexer_peek_at(lexer, tokens, p5);
    cp_lx = plant_iLexer_tok_lex(lexer, cp_tok);
    if (strcmp(cp_lx,"(") == 0) {
    cp_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(cp_pair);
    has_cparen = "1";
    }
    PlantArray* args = plant_list_make ( 0 );
    ...
    while (1) {
    tok0 = plant_iLexer_peek_at(lexer, tokens, p5);
    lx0 = plant_iLexer_tok_lex(lexer, tok0);
    ty0 = plant_iLexer_tok_type(lexer, tok0);
    if (strcmp(lx0,",") == 0 && strcmp(ty0,"STRING") != 0) {
    com0 = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(com0);
    }
    ctok = plant_iLexer_peek_at(lexer, tokens, p5);
    clx = plant_iLexer_tok_lex(lexer, ctok);
    if (strcmp(clx,"[") == 0) {
    ...
    clx2 = plant_iLexer_tok_lex(lexer, ctok2);
    if (strcmp(clx2,",") == 0) {
    ccom = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(ccom);
    }
    if (strcmp(clx2,".") == 0) {
    cdot = plant_iLexer_consume_at(lexer, tokens, p5);
    p6 = _second(cdot);
    ...
    ty = plant_iLexer_tok_type(lexer, tok);
    if (strcmp(lx,"IN") == 0 && strcmp(ty,"STRING") != 0) {
    in_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5b = _second(in_pair);
    ctx_pair = plant_iLexer_consume_at(lexer, tokens, p5b);
    ctx_name = plant_iLexer_tok_lex(lexer, plant_list_get(ctx_pair ,  0 ));
    p6b = _second(ctx_pair);
    dot2 = plant_iLexer_consume_at(lexer, tokens, p6b);
    p7b = _second(dot2);
    ...
    rp = plant_iLexer_consume_at(lexer, tokens, p5);
    p5r = _second(rp);
    in_tok = plant_iLexer_peek_at(lexer, tokens, p5r);
    in_lx = plant_iLexer_tok_lex(lexer, in_tok);
    in_ty = plant_iLexer_tok_type(lexer, in_tok);
    if (strcmp(in_lx,"IN") == 0 && strcmp(in_ty,"STRING") != 0) {
    in_pair = plant_iLexer_consume_at(lexer, tokens, p5r);
    p5r = _second(in_pair);
    ctx_pair = plant_iLexer_consume_at(lexer, tokens, p5r);
    ctx_name = plant_iLexer_tok_lex(lexer, plant_list_get(ctx_pair ,  0 ));
    p5r = _second(ctx_pair);
    dot3 = plant_iLexer_consume_at(lexer, tokens, p5r);
    p5r = _second(dot3);
...
tx_t is_reap_builtin(tx_t name) {
  tx_t lexer = "";
    ...
    if (strcmp(name,"MIN") == 0 || strcmp(name,"MAX") == 0 || strcmp(name,"TAN") == 0 || strcmp(name,"ATAN") == 0 || strcmp(name,"COT") == 0 || strcmp(name,"ASIN") == 0 || strcmp(name,"ACOS") == 0 || strcmp(name,"ATAN2") == 0 || strcmp(name,"SINH") == 0 || strcmp(name,"COSH") == 0 || strcmp(name,"TANH") == 0 || strcmp(name,"EXP") == 0 || strcmp(name,"EXPM1") == 0 || strcmp(name,"LOG10") == 0 || strcmp(name,"LOG2") == 0 || strcmp(name,"LOG1P") == 0 || strcmp(name,"HYPOT") == 0) {
    return "1";
    }
...
tx_t parse_call_stmt(PlantArray* tokens, long pos, PlantArray* rtab, tx_t emode) {
  ...
  tx_t com = "";
    lexer = get_lexer();
    act_pair = plant_iLexer_consume_at(lexer, tokens, pos);
    act_name = plant_iLexer_tok_lex(lexer, plant_list_get(act_pair ,  0 ));
    p5 = _second(act_pair);
    next_tok = plant_iLexer_peek_at(lexer, tokens, p5);
    next_ty = plant_iLexer_tok_type(lexer, next_tok);
    ...
    lpar = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(lpar);
    PlantArray* args = plant_list_make ( 0 );
    PlantArray* clargs = plant_list_make ( 0 );
    while (1) {
...
tx_t parse_put_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p6 = "";
    ...
    p3 = _second(vpair);
    into_pair = plant_iLexer_consume_at(lexer, tokens, p3);
...
tx_t parse_braid_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p7 = "";
    ...
    p3 = _second(lpair);
    with_pair = plant_iLexer_consume_at(lexer, tokens, p3);
    p4 = _second(with_pair);
    rpair = collect_until(tokens, p4, "AS");
    tx_t right = plant_list_get(rpair ,  0 );
    p5 = _second(rpair);
    as_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p6 = _second(as_pair);
    tx_t target = "";
    while (1) {
    tok = plant_iLexer_peek_at(lexer, tokens, p6);
    lx = plant_iLexer_tok_lex(lexer, tok);
    if (strcmp(lx,"MAP") == 0 || strcmp(lx,".") == 0 || strcmp(lx,"") == 0) {
                      break;
    }
    target = _cat(target, lx);
    drop = plant_iLexer_consume_at(lexer, tokens, p6);
    p6 = _second(drop);
    }
    tx_t is_map = "0";
    tok = plant_iLexer_peek_at(lexer, tokens, p6);
...
tx_t parse_link_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p8 = "";
    ...
    p7 = _second(mpair);
    dot_pair = plant_iLexer_consume_at(lexer, tokens, p7);
    p8 = _second(dot_pair);
...
tx_t parse_sort_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p3 = "";
    ...
    target = _cat(target, lx);
    drop = plant_iLexer_consume_at(lexer, tokens, p2);
    p2 = _second(drop);
    }
    tok = plant_iLexer_peek_at(lexer, tokens, p2);
    lx = plant_iLexer_tok_lex(lexer, tok);
    ...
    p2 = _second(drop);
    tok = plant_iLexer_peek_at(lexer, tokens, p2);
    lx2 = plant_iLexer_tok_lex(lexer, tok);
    ...
    dot_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    p3 = _second(dot_pair);
...
tx_t parse_if_stmt(PlantArray* tokens, long pos, tx_t clv, PlantArray* rtab, tx_t emode) {
  ...
  tx_t sty = "";
    ...
    p3 = _second(cpair);
    com = plant_iLexer_consume_at(lexer, tokens, p3);
    p4 = _second(com);
    ...
    lx = plant_iLexer_tok_lex(lexer, tok);
    if (strcmp(lx,"/") == 0) {
    slash = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(slash);
    if_close = plant_iLexer_consume_at(lexer, tokens, p5);
    p6 = _second(if_close);
    dot = plant_iLexer_consume_at(lexer, tokens, p6);
    p7 = _second(dot);
    if (strcmp(in_elif,"1") == 0) {
    ...
    o_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(o_pair);
    ocp = collect_until(tokens, p5, ",");
    ocond = _first(ocp);
    p6 = _second(ocp);
    ocom = plant_iLexer_consume_at(lexer, tokens, p6);
    p7 = _second(ocom);
    cur_cond = ocond;
    ...
    e_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(e_pair);
    in_else = "1";
    p4 = p5;
    ctab = plant_list_make ( 0 );
...
tx_t parse_match_stmt(PlantArray* tokens, long pos, tx_t clv, PlantArray* rtab, tx_t emode) {
  ...
  tx_t dcons = "";
    ...
    pcons = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(pcons);
    }
    ...
    pcons = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(pcons);
    ntok = plant_iLexer_peek_at(lexer, tokens, p4);
    ntt = plant_iLexer_tok_type(lexer, ntok);
    if (strcmp(ntt,"LPAREN") == 0) {
    ntok2 = plant_iLexer_peek_at(lexer, tokens, p4+1);
    ntt2 = plant_iLexer_tok_type(lexer, ntok2);
    if (strcmp(ntt2,"IDENT") != 0) {
    return plant_list_make ( 2 , NULL , p4 );
    }
    tx_t bnd = plant_list_get(ntok2 ,  1 );
    lpair = plant_iLexer_consume_at(lexer, tokens, p4);
    p4b = _second(lpair);
    bpair = plant_iLexer_consume_at(lexer, tokens, p4b);
    p4c = _second(bpair);
    rpair = plant_iLexer_consume_at(lexer, tokens, p4c);
    p4 = _second(rpair);
    ptext = lx;
    pbind = bnd;
    }
    ...
    ar = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(ar);
    PlantArray* ctab = plant_list_make ( 0 );
    long bstart = p4;
    PlantArray* cbody = plant_list_make ( 0 );
    btok = plant_iLexer_peek_at(lexer, tokens, p4);
    blx = plant_iLexer_tok_lex(lexer, btok);
    if (strcmp(blx,"{") == 0) {
    lbg = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(lbg);
    while (1) {
    is_eof2 = plant_iLexer_is_eof_at(lexer, tokens, p4);
    if (is_eof2) {
                                      break;
    }
    btok2 = plant_iLexer_peek_at(lexer, tokens, p4);
    blx2 = plant_iLexer_tok_lex(lexer, btok2);
    if (strcmp(blx2,"}") == 0) {
    rbg = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(rbg);
                                      break;
    }
    ...
    dotok = plant_iLexer_peek_at(lexer, tokens, p4);
    dotlx = plant_iLexer_tok_lex(lexer, dotok);
    if (strcmp(dotlx,".") == 0) {
    dcons = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(dcons);
    }
...
tx_t parse_season_stmt(PlantArray* tokens, long pos, tx_t clv, PlantArray* rtab, tx_t emode) {
  ...
  tx_t sty = "";
    ...
    slash = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(slash);
    season_close = plant_iLexer_consume_at(lexer, tokens, p5);
...
tx_t parse_cycle_stmt(PlantArray* tokens, long pos, tx_t clv, PlantArray* rtab, tx_t emode) {
  ...
  tx_t sty = "";
    ...
    p2 = _second(pair);
    tok = plant_iLexer_peek_at(lexer, tokens, p2);
    ivar = plant_iLexer_tok_lex(lexer, tok);
    cpair = plant_iLexer_consume_at(lexer, tokens, p2);
    p2 = _second(cpair);
    tx_t fromExpr = "";
    tx_t toExpr = "";
    ...
    p3 = _second(cpair);
    cpair = plant_iLexer_consume_at(lexer, tokens, p3);
    p3 = _second(cpair);
    cpair = collect_until_keyword(tokens, p3, plant_list_make ( 2 , "STEP" , "," ));
    toExpr = plant_list_get(cpair ,  0 );
    p4 = _second(cpair);
    stok = plant_iLexer_peek_at(lexer, tokens, p4);
    slx = plant_iLexer_tok_lex(lexer, stok);
    if (strcmp(slx,"STEP") == 0) {
    cpair = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(cpair);
    ...
    p2 = _second(cpair);
    tok = plant_iLexer_peek_at(lexer, tokens, p2);
    indexVar = plant_iLexer_tok_lex(lexer, tok);
    cpair = plant_iLexer_consume_at(lexer, tokens, p2);
    p2 = _second(cpair);
    tok = plant_iLexer_peek_at(lexer, tokens, p2);
    lx2 = plant_iLexer_tok_lex(lexer, tok);
    ...
    slash = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(slash);
    cclose = plant_iLexer_consume_at(lexer, tokens, p5);
    p6 = _second(cclose);
    dot = plant_iLexer_consume_at(lexer, tokens, p6);
    p7 = _second(dot);
...
tx_t parse_throw_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p4 = "";
    ...
    pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(pair);
    t_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    p3 = _second(t_pair);
    stype = plant_iLexer_tok_lex(lexer, plant_list_get(t_pair ,  0 ));
    tx_t msg = "";
    tok = plant_iLexer_peek_at(lexer, tokens, p3);
    lx = plant_iLexer_tok_lex(lexer, tok);
    if (strcmp(lx,"(") == 0) {
    ...
    dot = plant_iLexer_consume_at(lexer, tokens, p3);
    p4 = _second(dot);
...
tx_t parse_stop_if_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p5 = "";
    ...
    pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(pair);
    if_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    if_lx = plant_iLexer_tok_lex(lexer, plant_list_get(if_pair ,  0 ));
    p3 = _second(if_pair);
...
tx_t parse_weather_stmt(PlantArray* tokens, long pos, tx_t clv, PlantArray* rtab, tx_t emode) {
  ...
  tx_t sty = "";
    ...
    lx = plant_iLexer_tok_lex(lexer, tok);
    if (strcmp(lx,",") == 0) {
    c_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    p2 = _second(c_pair);
    }
    PlantArray* body = plant_list_make ( 0 );
    ...
    slash = plant_iLexer_consume_at(lexer, tokens, p2);
    p3 = _second(slash);
    cclose = plant_iLexer_consume_at(lexer, tokens, p3);
    p4 = _second(cclose);
    clx = plant_iLexer_tok_lex(lexer, plant_list_get(cclose ,  0 ));
    ...
    s_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    p3 = _second(s_pair);
    ...
    lx2 = plant_iLexer_tok_lex(lexer, tok2);
    if (strcmp(lx2,",") != 0) {
    t_pair = plant_iLexer_consume_at(lexer, tokens, p3);
    p3 = _second(t_pair);
    stype = plant_iLexer_tok_lex(lexer, plant_list_get(t_pair ,  0 ));
    tok3 = plant_iLexer_peek_at(lexer, tokens, p3);
    lx3 = plant_iLexer_tok_lex(lexer, tok3);
    if (strcmp(lx3,"AS") == 0) {
    a_pair = plant_iLexer_consume_at(lexer, tokens, p3);
    p3 = _second(a_pair);
    b_pair = plant_iLexer_consume_at(lexer, tokens, p3);
    p3 = _second(b_pair);
    sbind = plant_iLexer_tok_lex(lexer, plant_list_get(b_pair ,  0 ));
    }
    }
    tok4 = plant_iLexer_peek_at(lexer, tokens, p3);
    lx4 = plant_iLexer_tok_lex(lexer, tok4);
    if (strcmp(lx4,",") == 0) {
    cm_pair = plant_iLexer_consume_at(lexer, tokens, p3);
    p3 = _second(cm_pair);
    }
    cur_type = stype;
    cur_bind = sbind;
...
tx_t parse_statement(PlantArray* tokens, long pos, tx_t clv, PlantArray* ctab, PlantArray* rtab, long bstart, tx_t emode) {
  ...
  tx_t nx3_ty = "";
    lexer = get_lexer();
    tok = plant_iLexer_peek_at(lexer, tokens, pos);
    lx = plant_iLexer_tok_lex(lexer, tok);
    if (strcmp(lx,"TYPE") == 0) {
    r = parse_type_decl(tokens, pos);
    return r;
    }
    if (strcmp(lx,"BLOOM") == 0) {
    r = parse_bloom_stmt(tokens, pos);
    return r;
    }
    ...
    r = parse_array_decl(tokens, pos, rtab);
    return r;
    }
    if (strcmp(lx,"SHOW") == 0) {
    ...
    if (strcmp(lx,"VERIFY") == 0) {
    r = parse_verify_stmt(tokens, pos);
    return r;
    }
    ...
    if (strcmp(lx,"SUITE") == 0) {
    r = parse_suite_stmt(tokens, pos, clv, ctab, rtab, bstart, emode);
    return r;
    }
    if (strcmp(lx,"SETUP") == 0) {
    r = parse_setup_stmt(tokens, pos);
    return r;
    }
    if (strcmp(lx,"TEARDOWN") == 0) {
    r = parse_teardown_stmt(tokens, pos);
    return r;
    }
    nx = plant_iLexer_peek_at(lexer, tokens, pos+1);
    nx_ty = plant_iLexer_tok_type(lexer, nx);
    if (strcmp(nx_ty,"LPAREN") == 0) {
    r = parse_call_stmt(tokens, pos, rtab, emode);
    return r;
    }
    if (strcmp(nx_ty,"DOT") == 0) {
    mm1 = plant_iLexer_peek_at(lexer, tokens, pos+2);
    mm1_ty = plant_iLexer_tok_type(lexer, mm1);
    if (strcmp(mm1_ty,"IDENT") == 0) {
    mm2 = plant_iLexer_peek_at(lexer, tokens, pos+3);
    mm2_ty = plant_iLexer_tok_type(lexer, mm2);
    ...
    nx2 = plant_iLexer_peek_at(lexer, tokens, pos+1);
    nx2_ty = plant_iLexer_tok_type(lexer, nx2);
    if (strcmp(nx2_ty,"COLON") == 0) {
    nx3 = plant_iLexer_peek_at(lexer, tokens, pos+2);
    nx3_ty = plant_iLexer_tok_type(lexer, nx3);
...
tx_t parse_harvest_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p5 = "";
    ...
    if (strcmp(lx,"MAP") == 0) {
    dropm = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(dropm);
    map = "1";
                      continue;
    }
    if (strcmp(lx,"JSON") == 0) {
    dropj = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(dropj);
    json = "1";
                      continue;
    }
    if (strcmp(lx,"METHOD") == 0 || strcmp(lx,"BODY") == 0 || strcmp(lx,"HEADERS") == 0 || strcmp(lx,"TIMEOUT") == 0) {
    drop = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(drop);
    ...
    lx2 = plant_iLexer_tok_lex(lexer, tok2);
    if (strcmp(lx2,",") == 0) {
    drop2 = plant_iLexer_consume_at(lexer, tokens, p4);
    p4 = _second(drop2);
    }
                      continue;
    }
    ...
    dot_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(dot_pair);
...
tx_t parse_listen_stmt(PlantArray* tokens, long pos) {
  ...
  tx_t p7 = "";
    ...
    pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(pair);
    on_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    p3 = _second(on_pair);
    PlantArray* okws2 = plant_list_make ( 3 , "AS" , ")" , "." );
    opair = collect_until_keyword(tokens, p3, okws2);
    tx_t port = plant_list_get(opair ,  0 );
    p4 = _second(opair);
    as_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(as_pair);
    id_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    reqname = plant_iLexer_tok_lex(lexer, plant_list_get(id_pair ,  0 ));
    p6 = _second(id_pair);
    tx_t tmo = "";
    tok_t = plant_iLexer_peek_at(lexer, tokens, p6);
    lx_t = plant_iLexer_tok_lex(lexer, tok_t);
    if (strcmp(lx_t,"TIMEOUT") == 0) {
    drop_t = plant_iLexer_consume_at(lexer, tokens, p6);
    p6 = _second(drop_t);
...
tx_t parse_enum_decl(PlantArray* tokens, long pos) {
  ...
  tx_t com = "";
    ...
    name = plant_iLexer_tok_lex(lexer, plant_list_get(name_pair ,  0 ));
    p3 = _second(name_pair);
    lbr = plant_iLexer_consume_at(lexer, tokens, p3);
    p4 = _second(lbr);
    ...
    lx = plant_iLexer_tok_lex(lexer, tok);
    if (strcmp(lx,"}") == 0) {
    rbr = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(rbr);
    return plant_list_make ( 2 , plant_list_make ( 6 , "type" , "enum_decl" , "name" , name , "members" , members ) , p5 );
    }
    m_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    mname = plant_iLexer_tok_lex(lexer, plant_list_get(m_pair ,  0 ));
    p5 = _second(m_pair);
...
tx_t parse_struct_decl(PlantArray* tokens, long pos) {
  ...
  tx_t com = "";
    ...
    pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(pair);
    sg_tok0 = plant_iLexer_peek_at(lexer, tokens, p2);
    sg_lx0 = plant_iLexer_tok_lex(lexer, sg_tok0);
    tx_t sname = "";
    long p3 = p2;
    if (strcmp(sg_lx0,"{") != 0) {
    name_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    sname = plant_iLexer_tok_lex(lexer, plant_list_get(name_pair ,  0 ));
    p3 = _second(name_pair);
    }
    PlantArray* generics = plant_list_make ( 0 );
    sg_tok = plant_iLexer_peek_at(lexer, tokens, p3);
    sg_lx = plant_iLexer_tok_lex(lexer, sg_tok);
    if (strcmp(sg_lx,"[") == 0) {
    sg_lb = plant_iLexer_consume_at(lexer, tokens, p3);
    p3 = _second(sg_lb);
    sgv = collect_type_text(tokens, p3, "]", 0);
    sgtext = _first(sgv);
    p3 = _second(sgv);
    sg_rb = plant_iLexer_consume_at(lexer, tokens, p3);
    p3 = _second(sg_rb);
...
tx_t parse_action_decl(PlantArray* tokens, long pos, PlantArray* rtab) {
  ...
  tx_t slv = "";
    ...
    p2 = _second(pair);
    name_pair = plant_iLexer_consume_at(lexer, tokens, p2);
    aname = plant_iLexer_tok_lex(lexer, plant_list_get(name_pair ,  0 ));
    p3 = _second(name_pair);
    tx_t clv = "";
    tx_t prio = "1";
    ...
    long closep = - 1;
    PlantArray* generics = plant_list_make ( 0 );
    ga_tok = plant_iLexer_peek_at(lexer, tokens, p3);
    ga_lx = plant_iLexer_tok_lex(lexer, ga_tok);
    if (strcmp(ga_lx,"[") == 0) {
    ga_lb = plant_iLexer_consume_at(lexer, tokens, p3);
    p3 = _second(ga_lb);
    gv = collect_type_text(tokens, p3, "]", 0);
    gtext = _first(gv);
    p3 = _second(gv);
    ga_rb = plant_iLexer_consume_at(lexer, tokens, p3);
    p3 = _second(ga_rb);
    ...
    lx = plant_iLexer_tok_lex(lexer, tok);
    if (strcmp(lx,"..") == 0) {
    tokv2 = plant_iLexer_peek_at(lexer, tokens, p4+1);
    lxv2 = plant_iLexer_tok_lex(lexer, tokv2);
    if (strcmp(lxv2,".") == 0) {
    vararg = "1";
    p4 = p4+2;
    ctok2 = plant_iLexer_peek_at(lexer, tokens, p4);
    clx2 = plant_iLexer_tok_lex(lexer, ctok2);
    if (strcmp(clx2,")") == 0) {
    p4 = p4+1;
    }
    ...
    if (strcmp(lxv2,".") == 0) {
    tokv3 = plant_iLexer_peek_at(lexer, tokens, p4+2);
    lxv3 = plant_iLexer_tok_lex(lexer, tokv3);
    if (strcmp(lxv3,".") == 0) {
    vararg = "1";
    p4 = p4+3;
    ctok2 = plant_iLexer_peek_at(lexer, tokens, p4);
    ...
    if (strcmp(lx,")") == 0) {
    rp = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(rp);
    long brk = 0;
                      break;
    }
    pn_pair = plant_iLexer_consume_at(lexer, tokens, p4);
    pn = plant_iLexer_tok_lex(lexer, plant_list_get(pn_pair ,  0 ));
    p5 = _second(pn_pair);
    tok2 = plant_iLexer_peek_at(lexer, tokens, p5);
    lx2 = plant_iLexer_tok_lex(lexer, tok2);
    if (strcmp(lx2,"(") == 0) {
    lp2 = plant_iLexer_consume_at(lexer, tokens, p5);
    p6 = _second(lp2);
    ...
    pt = strings_REPLACE(pt, "* ", "*");
    rp2 = plant_iLexer_consume_at(lexer, tokens, p7);
    p7 = _second(rp2);
            params = plant_list_add(params, plant_list_make ( 4 , "name" , pn , "type" , pt ));
    p5 = p7;
    }
    ...
    tok3 = plant_iLexer_peek_at(lexer, tokens, p5);
    lx3 = plant_iLexer_tok_lex(lexer, tok3);
    if (strcmp(lx3,",") == 0) {
    com = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(com);
    ...
    tx_t ret = "";
    arrow_tok = plant_iLexer_peek_at(lexer, tokens, p5);
    arrow_lx = plant_iLexer_tok_lex(lexer, arrow_tok);
    if (strcmp(arrow_lx,"->") == 0) {
    arrow_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(arrow_pair);
    skip_tok = plant_iLexer_peek_at(lexer, tokens, p5);
    skip_ty = plant_iLexer_tok_type(lexer, skip_tok);
    if (strcmp(skip_ty,"MINUS") == 0) {
    skip_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(skip_pair);
    }
    ret_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    ret_lx = plant_iLexer_tok_lex(lexer, plant_list_get(ret_pair ,  0 ));
    p5 = _second(ret_pair);
    ...
    vst = plant_iLexer_peek_at(lexer, tokens, p5);
    vsl = plant_iLexer_tok_lex(lexer, vst);
    if (strcmp(vsl,"*") == 0) {
    vsp = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(vsp);
    ret = "void*";
    }
    }
    if (strcmp(ret_lx,"Result") == 0) {
    lt_tok = plant_iLexer_peek_at(lexer, tokens, p5);
    lt_lx = plant_iLexer_tok_lex(lexer, lt_tok);
    if (strcmp(lt_lx,"<") == 0) {
    lt_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(lt_pair);
    rt_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(rt_pair);
    rc_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(rc_pair);
    re_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(re_pair);
    gt_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(gt_pair);
    }
    }
    ...
    pr_tok = plant_iLexer_peek_at(lexer, tokens, p5);
    pr_lx = plant_iLexer_tok_lex(lexer, pr_tok);
    if (strcmp(pr_lx,"PRIORITY") == 0) {
    pr_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(pr_pair);
    lv_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    lv_lx = plant_iLexer_tok_lex(lexer, plant_list_get(lv_pair ,  0 ));
    p5 = _second(lv_pair);
    ...
    wm_tok = plant_iLexer_peek_at(lexer, tokens, p5);
    wm_lx = plant_iLexer_tok_lex(lexer, wm_tok);
    if (strcmp(wm_lx,"WITH") == 0) {
    wm_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(wm_pair);
    ms_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    ms_lx = plant_iLexer_tok_lex(lexer, plant_list_get(ms_pair ,  0 ));
    p5 = _second(ms_pair);
    if (strcmp(ms_lx,"MISSION") == 0) {
    md_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    md_lx = plant_iLexer_tok_lex(lexer, plant_list_get(md_pair ,  0 ));
    p5 = _second(md_pair);
    mission_mode = md_lx;
    }
    }
    after_tok = plant_iLexer_peek_at(lexer, tokens, p5);
    after_lx = plant_iLexer_tok_lex(lexer, after_tok);
    if (strcmp(after_lx,",") == 0) {
    com_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(com_pair);
    }
    if (strcmp(after_lx,".") == 0) {
    dot_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    ...
    if (strcmp(after_lx,"{") == 0) {
    lb2_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(lb2_pair);
    bracemode = "1";
    long bd2 = 1;
    long sp2 = p5;
    while (sp2 < plant_array_length(tokens)) {
    tk2 = plant_iLexer_peek_at(lexer, tokens, sp2);
    ty2 = plant_iLexer_tok_type(lexer, tk2);
    ...
    tk9 = plant_iLexer_peek_at(lexer, tokens, p5);
    ty9 = plant_iLexer_tok_type(lexer, tk9);
    if (strcmp(ty9,"RBRACE") == 0) {
    rb9_pair = plant_iLexer_consume_at(lexer, tokens, p5);
    p5 = _second(rb9_pair);
    ...
    tok4 = plant_iLexer_peek_at(lexer, tokens, p5);
    lx4 = plant_iLexer_tok_lex(lexer, tok4);
    if (strcmp(lx4,"/") == 0) {
    slash = plant_iLexer_consume_at(lexer, tokens, p5);
    p6 = _second(slash);
    end = plant_iLexer_consume_at(lexer, tokens, p6);
    p7 = _second(end);
    dot = plant_iLexer_consume_at(lexer, tokens, p7);
    p8 = _second(dot);
...
tx_t parse_species_decl(PlantArray* tokens, long pos) {
  ...
  tx_t brb = "";
    ...
    p = _second(kw_pair);
    npair = plant_iLexer_consume_at(lexer, tokens, p);
    nty = plant_iLexer_tok_type(lexer, plant_list_get(npair ,  0 ));
    if (strcmp(nty,"IDENT") != 0) {
    tx_t serr = "SPECIES requires a name";
    return plant_list_make ( 2 , plant_list_make ( 4 , "type" , "syntax_error" , "msg" , serr ) , p );
    }
    sname = plant_iLexer_tok_lex(lexer, plant_list_get(npair ,  0 ));
    p = _second(npair);
    PlantArray* stparams = plant_list_make ( 0 );
    sgtk = plant_iLexer_peek_at(lexer, tokens, p);
    sglx = plant_iLexer_tok_lex(lexer, sgtk);
    if (strcmp(sglx,"[") == 0) {
    sglb = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(sglb);
    sgv = collect_type_text(tokens, p, "]", 0);
    p = _second(sgv);
    sgt = _first(sgv);
    sgrb = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(sgrb);
    ...
    impl_tk = plant_iLexer_peek_at(lexer, tokens, p);
    impl_lx = plant_iLexer_tok_lex(lexer, impl_tk);
    if (strcmp(impl_lx,"IMPLEMENTS") == 0) {
    impl_c1 = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(impl_c1);
    impl_n1 = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(impl_n1);
    cm1t = plant_iLexer_peek_at(lexer, tokens, p);
    cm1l = plant_iLexer_tok_lex(lexer, cm1t);
    if (strcmp(cm1l,",") == 0) {
    cc9a = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(cc9a);
    in9a = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(in9a);
            implifs = plant_list_add(implifs, in9a);
    cm2t = plant_iLexer_peek_at(lexer, tokens, p);
    cm2l = plant_iLexer_tok_lex(lexer, cm2t);
    if (strcmp(cm2l,",") == 0) {
    cc9b = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(cc9b);
    in9b = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(in9b);
    ...
    tx_t parent = "";
    ftk = plant_iLexer_peek_at(lexer, tokens, p);
    flx9 = plant_iLexer_tok_lex(lexer, ftk);
    if (strcmp(flx9,"FROM") == 0) {
    fpair = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(fpair);
    ppair = plant_iLexer_consume_at(lexer, tokens, p);
    pnm = plant_iLexer_tok_lex(lexer, plant_list_get(ppair ,  0 ));
    parent = pnm;
    p = _second(ppair);
    }
    lb = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(lb);
    PlantArray* fields = plant_list_make ( 0 );
    ...
    tk = plant_iLexer_peek_at(lexer, tokens, p);
    ty = plant_iLexer_tok_type(lexer, tk);
    slx = plant_iLexer_tok_lex(lexer, tk);
    if (strcmp(ty,"RBRACE") == 0) {
    rb = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(rb);
                      break;
    }
    if (strcmp(slx,"ACTION") == 0) {
    mpair = parse_action_decl(tokens, p, srtab);
    mnode = _first(mpair);
    p = _second(mpair);
    mnty = _map_get(mnode, "type");
    if (strcmp(mnty,"action_decl") == 0) {
    mn = _map_get(mnode, "name");
    ...
    pel = plant_list_get(oldp, ci2);
                    np2 = plant_list_add(np2, pel);
    ci2 = ci2+1;
    }
    nn2 = _map_replace(onm, "params", np2);
                methods = plant_list_add(methods, nn2);
    }
    stk9 = plant_iLexer_peek_at(lexer, tokens, p);
    sty9 = plant_iLexer_tok_type(lexer, stk9);
    if (strcmp(sty9,"RBRACE") == 0) {
    rb9 = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(rb9);
                              break;
    }
                      continue;
    ...
    fpair = plant_iLexer_consume_at(lexer, tokens, p);
    fname = plant_iLexer_tok_lex(lexer, plant_list_get(fpair ,  0 ));
    p = _second(fpair);
    cpa9 = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(cpa9);
    ttok = plant_iLexer_consume_at(lexer, tokens, p);
    ftt = plant_iLexer_tok_lex(lexer, plant_list_get(ttok ,  0 ));
    p = _second(ttok);
    btk = plant_iLexer_peek_at(lexer, tokens, p);
    blx = plant_iLexer_tok_lex(lexer, btk);
    if (strcmp(blx,"[") == 0) {
    btv = collect_type_text(tokens, p, "]", 0);
    btx = _first(btv);
    p = _second(btv);
    ftt = _cat3(ftt, btx, "]");
    brb = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(brb);
    }
...
tx_t parse_type_decl(PlantArray* tokens, long pos) {
  ...
  tx_t dpair = "";
    ...
    kw_pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p = _second(kw_pair);
    apair = plant_iLexer_consume_at(lexer, tokens, p);
    aty = plant_iLexer_tok_type(lexer, plant_list_get(apair ,  0 ));
    ...
    alias = plant_iLexer_tok_lex(lexer, plant_list_get(apair ,  0 ));
    p = _second(apair);
    eqtok = plant_iLexer_peek_at(lexer, tokens, p);
    eqlx = plant_iLexer_tok_lex(lexer, eqtok);
    if (strcmp(eqlx,"=") != 0) {
    tx_t terr2 = "TYPE requires = target";
    return plant_list_make ( 2 , plant_list_make ( 4 , "type" , "syntax_error" , "msg" , terr2 ) , p );
    }
    eqpair = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(eqpair);
    tv = collect_type_text(tokens, p, ".", 0);
    target = _first(tv);
    p = _second(tv);
    dpair = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(dpair);
...
tx_t parse_interface_decl(PlantArray* tokens, long pos) {
  ...
  tx_t dcon = "";
    ...
    p = _second(kw_pair);
    npair = plant_iLexer_consume_at(lexer, tokens, p);
    iname = plant_iLexer_tok_lex(lexer, plant_list_get(npair ,  0 ));
    p = _second(npair);
    PlantArray* itparams = plant_list_make ( 0 );
    igtk = plant_iLexer_peek_at(lexer, tokens, p);
    iglx = plant_iLexer_tok_lex(lexer, igtk);
    if (strcmp(iglx,"[") == 0) {
    iglb = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(iglb);
    igv = collect_type_text(tokens, p, "]", 0);
    p = _second(igv);
    igt = _first(igv);
    igrb = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(igrb);
    ...
    iftk = plant_iLexer_peek_at(lexer, tokens, p);
    iflx = plant_iLexer_tok_lex(lexer, iftk);
    if (strcmp(iflx,"FROM") == 0) {
    ifp = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(ifp);
    while (1) {
    pip = plant_iLexer_consume_at(lexer, tokens, p);
    pin = plant_iLexer_tok_lex(lexer, plant_list_get(pip ,  0 ));
            iparents = plant_list_add(iparents, pin);
    pctk = plant_iLexer_peek_at(lexer, tokens, p);
    pclx = plant_iLexer_tok_lex(lexer, pctk);
    if (strcmp(pclx,",") == 0) {
    pcc = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(pcc);
    }
    if (strcmp(pclx,",") != 0) {
                              break;
    ...
    while (1) {
    tk9 = plant_iLexer_peek_at(lexer, tokens, p);
    ty9 = plant_iLexer_tok_type(lexer, tk9);
    if (strcmp(ty9,"RBRACE") == 0) {
    rb9 = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(rb9);
    ...
    mpair = plant_iLexer_consume_at(lexer, tokens, p);
    mnine = plant_iLexer_tok_lex(lexer, plant_list_get(mpair ,  0 ));
    p = _second(mpair);
        imethods = plant_list_add(imethods, mnine);
    dtok = plant_iLexer_peek_at(lexer, tokens, p);
    dlx = plant_iLexer_tok_lex(lexer, dtok);
    if (strcmp(dlx,".") == 0) {
    dcon = plant_iLexer_consume_at(lexer, tokens, p);
    p = _second(dcon);
                      break;
    }
    }
...
tx_t parse_declaration(PlantArray* tokens, long pos, tx_t clv, PlantArray* ctab, PlantArray* rtab, long bstart) {
  ...
  tx_t nd2 = "";
    ...
    if (strcmp(lx,"SPECIES") == 0) {
    r = parse_species_decl(tokens, pos);
    return r;
    }
    if (strcmp(lx,"IMPORT") == 0) {
    im_pair = plant_iLexer_consume_at(lexer, tokens, pos);
    im_p2 = _second(im_pair);
    stok = plant_iLexer_peek_at(lexer, tokens, im_p2);
    sty2 = plant_iLexer_tok_type(lexer, stok);
    ...
    spair = plant_iLexer_consume_at(lexer, tokens, im_p2);
    ipath = plant_iLexer_tok_lex(lexer, spair);
    im_p3 = _second(spair);
    dtok = plant_iLexer_peek_at(lexer, tokens, im_p3);
    dlx = plant_iLexer_tok_lex(lexer, dtok);
    if (strcmp(dlx,".") == 0) {
    dpair = plant_iLexer_consume_at(lexer, tokens, im_p3);
    im_p4 = _second(dpair);
    ...
    if (strcmp(lx,"ASYNC") == 0) {
    pair = plant_iLexer_peek_at(lexer, tokens, pos+1);
    nx = plant_iLexer_tok_lex(lexer, pair);
    if (strcmp(nx,"IN") == 0) {
    r = parse_async_in_stmt(tokens, pos);
    return r;
    }
...
tx_t scan_ident_used(PlantArray* tokens, long start, long end, tx_t name) {
  ...
  tx_t slx3 = "";
    ...
    tx_t stt3 = "";
    while (si3 < end) {
    stk3 = plant_iLexer_peek_at(lexer, tokens, si3);
    stt3 = plant_iLexer_tok_type(lexer, stk3);
    if (strcmp(stt3,"IDENT") == 0) {
    slx3 = plant_iLexer_tok_lex(lexer, stk3);
...
tx_t parse_const_like(PlantArray* tokens, long pos, PlantArray* ctab, PlantArray* rtab, long bstart, tx_t elevate, tx_t ntype) {
  ...
  tx_t nd2 = "";
    ...
    pair = plant_iLexer_consume_at(lexer, tokens, pos);
    p2 = _second(pair);
    nt = plant_iLexer_peek_at(lexer, tokens, p2);
    nty = plant_iLexer_tok_type(lexer, nt);
    if (strcmp(nty,"IDENT") != 0) {
    return plant_list_make ( 2 , plant_list_make ( 4 , "type" , "syntax_error" , "msg" , "CONST/ROOT requires an identifier name" ) , p2 );
    }
    npr = plant_iLexer_consume_at(lexer, tokens, p2);
    cname = plant_iLexer_tok_lex(lexer, plant_list_get(npr ,  0 ));
    p3 = _second(npr);
    ttok = plant_iLexer_peek_at(lexer, tokens, p3);
    tlx = plant_iLexer_tok_lex(lexer, ttok);
    if (strcmp(tlx,"TO") != 0) {
    return plant_list_make ( 2 , plant_list_make ( 4 , "type" , "syntax_error" , "msg" , "CONST/ROOT requires TO after the constant name" ) , p3 );
    }
    tp = plant_iLexer_consume_at(lexer, tokens, p3);
    p4 = _second(tp);
    vt = plant_iLexer_peek_at(lexer, tokens, p4);
    vty = plant_iLexer_tok_type(lexer, vt);
    if (strcmp(vty,"STRING") != 0) {
    return plant_list_make ( 2 , plant_list_make ( 4 , "type" , "syntax_error" , "msg" , "CONST/ROOT value must be a quoted string literal" ) , p4 );
    }
    vp = plant_iLexer_consume_at(lexer, tokens, p4);
    vraw = plant_iLexer_tok_lex(lexer, plant_list_get(vp ,  0 ));
    p5 = _second(vp);
    esc = escape_string(vraw);
    tx_t cval = _cat3("\"", esc, "\"");
    dt = plant_iLexer_peek_at(lexer, tokens, p5);
    dlx = plant_iLexer_tok_lex(lexer, dt);
    if (strcmp(dlx,".") != 0) {
    return plant_list_make ( 2 , plant_list_make ( 4 , "type" , "syntax_error" , "msg" , "CONST/ROOT expects '.' after the value" ) , p5 );
    }
    dp = plant_iLexer_consume_at(lexer, tokens, p5);
    p6 = _second(dp);
...
tx_t parse_root_scope_stmt(PlantArray* tokens, long pos, tx_t clv, PlantArray* rtab) {
  ...
  tx_t sty = "";
    ...
    slash = plant_iLexer_consume_at(lexer, tokens, p2);
    p3 = _second(slash);
    kp = plant_iLexer_consume_at(lexer, tokens, p3);
    klx = plant_iLexer_tok_lex(lexer, plant_list_get(kp ,  0 ));
    p4 = _second(kp);
    ...
    dp = plant_iLexer_consume_at(lexer, tokens, p4);
    p5 = _second(dp);
...
tx_t parse_program(PlantArray* tokens) {
  ...
  tx_t mn9 = "";
    ...
    while (1) {
    is_eof_flag = plant_iLexer_is_eof_at(lexer, tokens, pos);
    if (is_eof_flag) {
    ...
    mn9 = plant_list_get(ml, mi9);
                    nodes = plant_list_add(nodes, mn9);
    mi9 = mi9+1;
    }
    }
    if (strcmp(is_sp,"1") != 0) {
...
tx_t _handle_func_paren3(tx_t expr, tx_t kw, tx_t cfn) {
  ...
  tx_t wa3 = "";
    ...
    if (comma2 == - 1) {
    a1 = substring(inner, 0, comma1);
    a2 = substring(inner, comma1+1, strlen( inner ));
    wa1 = plant_trim(a1);
    wa2 = plant_trim(a2);
    res = _cat(_cat4(_cat4(res, cfn, "(", wa1), ", ", wa2, ")"), substring ( p , close_pos + 1 , strlen( p ) ));
    } else {
    a1 = substring(inner, 0, comma1);
    a2 = substring(inner, comma1+1, comma2);
    a3 = substring(inner, comma2+1, strlen( inner ));
    wa1 = plant_trim(a1);
    wa2 = plant_trim(a2);
    wa3 = plant_trim(a3);
    res = _cat4(_cat4(_cat4(res, cfn, "(", wa1), ", ", wa2, ", "), wa3, ")", substring ( p , close_pos + 1 , strlen( p ) ));
    }
    }
    idx = idx+1;
    }
...
tx_t _handle_func_paren4(tx_t expr, tx_t kw, tx_t cfn) {
  ...
  tx_t wa4 = "";
    ...
    a1 = substring(inner, 0, comma1);
    a2 = substring(inner, comma1+1, comma2);
    a3 = substring(inner, comma2+1, comma3);
    a4 = substring(inner, comma3+1, strlen( inner ));
    wa1 = plant_trim(a1);
    wa2 = plant_trim(a2);
    wa3 = plant_trim(a3);
    wa4 = plant_trim(a4);
    res = _cat3(_cat4(_cat4(_cat4(res, cfn, "(", wa1), ", ", wa2, ", "), wa3, ", ", wa4), ")", substring ( p , close_pos + 1 , strlen( p ) ));
    }
    }
    }
    idx = idx+1;
...
tx_t _handle_func_paren5(tx_t expr, tx_t kw, tx_t cfn) {
  ...
  tx_t wa5 = "";
    ...
    a4 = substring(inner, comma3+1, comma4);
    a5 = substring(inner, comma4+1, strlen( inner ));
    wa1 = plant_trim(a1);
    wa2 = plant_trim(a2);
    wa3 = plant_trim(a3);
    wa4 = plant_trim(a4);
    wa5 = plant_trim(a5);
    res = _cat(_cat4(_cat4(_cat4(_cat4(res, cfn, "(", wa1), ", ", wa2, ", "), wa3, ", ", wa4), ", ", wa5, ")"), substring ( p , close_pos + 1 , strlen( p ) ));
    }
    }
    }
    }
...
tx_t _is_num_literal(tx_t s) {
  ...
  tx_t c3 = "";
    ...
    if (strcmp(c3,".") != 0 && ( strcmp(c3,"0") < 0 || strcmp(c3,"9") > 0 )) {
    ok = 0;
    }
    if (strcmp(c3,"0") >= 0 && strcmp(c3,"9") <= 0) {
    digits = digits+1;
    }
    i = i+1;
    }
    if (ok == 1 && digits > 0 && dots < 2) {
    return "1";
    }
    return "0";
}
tx_t _wrap_math_args(tx_t e, tx_t cfn) {
  ...
  tx_t tail = "";
  tx_t rest = "";
    hpos = _find_substr(e, _cat(cfn, "("));
    if (hpos == - 1) {
    return e;
    }
    long open = plant_rw_arg_long ( hpos )+strlen( cfn );
    long i = open+1;
    long dep = 1;
    long close = - 1;
    while (i < strlen( e )) {
    ...
    inner = substring(e, open+1, close);
    PlantArray* parts = plant_list_make ( 0 );
    tx_t cur = "";
    long d2 = 0;
    long k = 0;
    long mm = strlen( inner );
    while (k < mm) {
    ck = char_at(inner, k);
    if (strcmp(ck,"(") == 0 || strcmp(ck,"[") == 0) {
    d2 = d2+1;
    }
    if (strcmp(ck,")") == 0 || strcmp(ck,"]") == 0) {
    d2 = d2 - 1;
    ...
    if (strcmp(ck,",") == 0 && d2 == 0) {
            parts = plant_list_add(parts, cur);
    cur = "";
    }
    if (strcmp(ck,",") != 0 || d2 != 0) {
    cur = _cat(cur, ck);
    ...
    while (idx < cn) {
    p = plant_list_get(parts, idx);
    isnum = _is_num_literal(p);
    if (idx > 0) {
    res = _cat(res, ", ");
    }
    if (strcmp(isnum,"1") == 0) {
    res = _cat4(res, "_from_double(", p, ")");
    }
    if (strcmp(isnum,"1") != 0) {
    res = _cat(res, p);
    }
    ...
    head = substring(e, 0, open+1);
    tail = substring(e, close, strlen( e ));
    rest = _wrap_math_args(tail, cfn);
    return _cat3(head, res, rest);
}
tx_t _math_func_paren(tx_t expr, tx_t kw, tx_t cfn) {
  tx_t e2 = "";
  tx_t e3 = "";
    e2 = _handle_func_paren(expr, kw, cfn);
    e3 = _wrap_math_args(e2, cfn);
    return e3;
}
tx_t _trim_sp(tx_t s) {
  tx_t c = "";
...
tx_t _kw_commacount(tx_t expr, tx_t kw) {
  ...
  tx_t ch = "";
    ...
    if (h2 != - 1) {
    open = _cat(h2, _from_long(strlen( kw )));
    }
    }
    if (open == - 1) {
    return "-1";
...
tx_t _range_once(tx_t e) {
  ...
  tx_t wrapped = "";
    ...
    isclose = str_eq(c3, qch);
    if (strcmp(isclose,"1") == 0) {
    instr = 0;
    }
    }
    if (instr == 0) {
    if (strcmp(c3,"(") == 0 || strcmp(c3,"[") == 0) {
    dep = dep+1;
    }
    if (strcmp(c3,")") == 0 || strcmp(c3,"]") == 0) {
    dep = dep - 1;
    }
    if (dep >= 0 && strcmp(c3,".") == 0) {
    c4 = char_at(e, i+1);
    if (strcmp(c4,".") == 0) {
    hit = i;
                                      break;
    ...
    if (strcmp(rc,",") == 0 && cur2 == dep) {
    rend = k;
                      break;
    }
    k = k+1;
    }
    lhs = _trim_sp(substring ( e , lstart , hit ));
    rhs = _trim_sp(substring ( e , hit + 2 , rend ));
    if (strcmp(lhs,"") == 0 || strcmp(rhs,"") == 0) {
    return e;
    }
    tx_t calltxt = "plant_range_list(";
    calltxt = _cat(calltxt, lhs);
    calltxt = _cat(calltxt, ", ");
    calltxt = _cat(calltxt, rhs);
    calltxt = _cat(calltxt, ")");
    wrapped = _wrap_math_args(calltxt, "plant_range_list");
    tx_t head = substring ( e , 0 , lstart );
    tx_t tail = substring ( e , rend , strlen( e ) );
    tx_t out = head;
    out = _cat(out, wrapped);
    out = _cat(out, tail);
    return out;
}
tx_t _find_substr(tx_t s, tx_t needle) {
...
tx_t _union_init(tx_t expr, PlantArray* nums, PlantArray* evars) {
  ...
  tx_t fval = "";
    ...
    before = substring(expr, 0, lp);
    before = trim(before);
    long blen = strlen( before );
    if (blen == 0) {
    return expr;
    }
    bc = char_at(before, blen - 1);
    if (strcmp(bc,")") == 0 || strcmp(bc,"]") == 0 || strcmp(bc,"\"") == 0) {
    return expr;
    }
    long bi = blen - 1;
    while (bi >= 0) {
    bc2 = char_at(before, bi);
    if (strcmp(bc2," ") == 0 || strcmp(bc2,"(") == 0 || strcmp(bc2,",") == 0 || strcmp(bc2,"+") == 0 || strcmp(bc2,"-") == 0 || strcmp(bc2,"*") == 0 || strcmp(bc2,"=") == 0) {
    return expr;
    }
    if (strcmp(bc2,"}") == 0 || strcmp(bc2,"]") == 0) {
    return expr;
    }
    bi = bi - 1;
    }
    tname = substring(before, bi+1, blen);
    iut = is_union_type(tname, evars);
    if (strcmp(iut,"0") == 0) {
    return expr;
    }
    inner = substring(expr, lp+1, ep);
    inner = trim(inner);
    colon_pos = find_any(inner, ":");
    if (colon_pos == - 1) {
    return expr;
    }
    fname = substring(inner, 0, colon_pos);
    fname = trim(fname);
    fval = substring(inner, colon_pos+1, strlen( inner ));
    fval = trim(fval);
    return _cat4(_cat4("(union ", tname, "){.", fname), " = ", fval, "}");
}
tx_t _map_literal(tx_t expr, PlantArray* nums, PlantArray* evars) {
  tx_t isub = "";
  tx_t pvi = "";
...
tx_t seg_is_numeric(tx_t seg, PlantArray* nums) {
  ...
  tx_t mf = "";
    ...
    pre9 = substring(sb, 0, 18);
    if (strcmp(pre9,"plant_rw_arg_long(") == 0) {
    return 1;
    }
    pre10 = substring(sb, 0, 13);
    if (strcmp(pre10,"plant_is_some(") == 0) {
    return 1;
    }
    pre11 = substring(sb, 0, 13);
    if (strcmp(pre11,"plant_is_none(") == 0) {
    return 1;
    }
    pre12 = substring(sb, 0, 11);
    if (strcmp(pre12,"plant_is_ok(") == 0) {
    return 1;
    }
...
tx_t _expand_bare(tx_t t, PlantArray* nums, PlantArray* evars) {
  ...
  tx_t inner = "";
    ...
    inner = translate_expr(inner, nums, evars, plant_list_make ( 0 ));
...
tx_t _interp_expand(tx_t raw, PlantArray* nums, PlantArray* evars) {
  ...
  tx_t out = "";
    ...
    seg2 = translate_expr(seg2, nums, evars, plant_list_make ( 0 ));
...
tx_t build_enum_registry(PlantArray* ast) {
  ...
  tx_t uname2 = "";
    ...
    tx_t un = "";
    tx_t uty = "";
    while (ui < plant_array_length(ast)) {
    un = plant_list_get(ast, ui);
    uty = _map_get(un, "type");
    if (strcmp(uty,"union_decl") == 0) {
    uname2 = _map_get(un, "name");
    PlantArray* ufldlst = _map_get ( un , "fields" );
    tx_t ufcsv = "";
    long ufi2 = 0;
    tx_t ufel = "";
    while (ufi2 < plant_array_length(ufldlst)) {
    ufel = plant_list_get(ufldlst, ufi2);
    if (ufi2 > 0) {
    ufcsv = _cat(ufcsv, ",");
    }
    ufcsv = _cat4(ufcsv, _map_get ( ufel , "name" ), ":", _map_get ( ufel , "type" ));
    ufi2 = ufi2+1;
    }
    tx_t urekey = _cat("UNION.", uname2);
            reg = plant_list_add(reg, urekey);
            reg = plant_list_add(reg, ufcsv);
    }
    ui = ui+1;
    }
    return reg;
...
tx_t collect_maths_walk(PlantArray* bd, PlantArray* subst, PlantArray* res) {
  ...
  tx_t mcb2 = "";
    ...
    while (ii4 < plant_array_length(ib)) {
    ibd4 = plant_list_get(ib, ii4);
    wret2 = collect_maths_walk(ibd4, subst, res);
    ii4 = ii4+1;
    }
    }
    if (strcmp(wty,"season_stmt") == 0 || strcmp(wty,"cycle_stmt") == 0) {
    wbd2 = _map_get(wnd, "body");
    wret2 = collect_maths_walk(wbd2, subst, res);
    }
    if (strcmp(wty,"match_stmt") == 0) {
    PlantArray* mcl3 = _map_get ( wnd , "clauses" );
    long mci3 = 0;
    ...
    mcl4 = plant_list_get(mcl3, mci3);
    mcb2 = _map_get(mcl4, "bodyStatements");
    wret2 = collect_maths_walk(mcb2, subst, res);
    mci3 = mci3+1;
    }
    }
    wi = wi+1;
...
tx_t collect_maths(PlantArray* bd, PlantArray* params, PlantArray* subst) {
  tx_t ret = "";
    ...
    psu2 = subst_type(pty2, subst);
    pbs2 = type_base(psu2);
    if (strcmp(pbs2,"MATH") == 0) {
            res = plant_list_add(res, pn2);
    }
    pi = pi+1;
    }
    ret = collect_maths_walk(bd, subst, res);
    return res;
}
tx_t _wrap_math_vars(tx_t e, PlantArray* maths) {
  tx_t chunk = "";
  ...
  tx_t sch = "";
    long mi = 0;
    tx_t mv = "";
    while (mi < plant_array_length(maths)) {
    mv = plant_list_get(maths, mi);
    tx_t wrapped = "";
    long pos = 0;
    long elen = strlen( e );
    long vlen = strlen( mv );
    while (pos < elen) {
    chunk = substring(e, pos, pos+vlen);
    chunk_eq = str_eq(chunk, mv);
    if (chunk_eq == 1) {
    long before_ok = 1;
    if (pos > 0) {
    bch = char_at(e, pos - 1);
    bisid = find_any(bch, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_");
    if (bisid != - 1) {
    before_ok = 0;
    }
    }
    long after_ok = 1;
                apos = _from_long(pos+vlen);
    if (apos < elen) {
    ach = char_at(e, apos);
    aisid = find_any(ach, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_(");
    if (aisid != - 1) {
    after_ok = 0;
    }
    }
    if (before_ok == 1 && after_ok == 1) {
    wrapped = _cat4(wrapped, "plant_math_eval(", mv, ")");
    pos = pos+vlen;
    } else {
    wrapped = _cat(wrapped, chunk);
    pos = pos+1;
    }
    } else {
    sch = char_at(e, pos);
    wrapped = _cat(wrapped, sch);
    pos = pos+1;
    }
    }
    e = wrapped;
...
tx_t async_argstr(PlantArray* args, PlantArray* sigs, tx_t act, PlantArray* nums, PlantArray* stvars, PlantArray* evars) {
  ...
  tx_t fs2 = "";
    ...
    if (strcmp(a0,"\"") != 0) {
    aex = translate_expr(ael, nums, evars, plant_list_make ( 0 ));
...
tx_t async_emit_step(tx_t name, PlantArray* phases, PlantArray* vars, PlantArray* sigs, PlantArray* subst, PlantArray* clmap, PlantArray* stvars, PlantArray* evars) {
  ...
  tx_t tcode = "";
    ...
    lcode = generate_body(lead, env_make ( 1 , sigs , subst , clmap , "a" , nums_s , stvars , evars , "" , "" , "" , plant_list_make ( 0 ) ));
    ...
    if (nxt == nph) {
    tcode = generate_body(phd, env_make ( 1 , sigs , subst , clmap , "a" , nums_s , stvars , evars , "" , "" , "" , plant_list_make ( 0 ) ));
...
tx_t _emit_ds(PlantArray* items, tx_t kind, tx_t base, tx_t isel, tx_t tag) {
  ...
  tx_t sitems = "";
    tx_t code = "";
    long i = 0;
    while (i < plant_array_length(items)) {
    it = plant_list_get(items, i);
    key = _map_get(it, "key");
    nm = _map_get(it, "name");
    sub = _map_get(it, "sub");
    tx_t ex = "";
    ...
    if (strcmp(kind,"object") != 0) {
    ex = _cat(ex, "plant_list_get(");
    ex = _cat(ex, base);
    ex = _cat(ex, ", ");
    ex = _cat(ex, _from_long ( i ));
    ex = _cat(ex, ")");
    }
    if (strcmp(sub,"") == 0) {
    if (strcmp(nm,"_") != 0) {
    code = _cat4(_cat4(code, isel, "  tx_t ", nm), " = ", ex, ";\n");
    }
    }
    if (strcmp(sub,"") != 0) {
    tx_t tname = _cat3(_cat4("_ds_", tag, "_", _from_long ( plant_unique_seq ( ) )), "_", _from_long ( i ));
    code = _cat4(_cat4(code, isel, "  tx_t ", tname), " = ", ex, ";\n");
    skind = _map_get(sub, "kind");
    sitems = _map_get(sub, "items");
    code = _cat(code, _emit_ds ( sitems , skind , tname , isel , tag ));
    }
    i = i+1;
    }
    return code;
...
tx_t _swap_self(tx_t e) {
  ...
  tx_t nc = "";
    ...
    while (i < n) {
    c = char_at(e, i);
    if (strcmp(c,"S") == 0 && i + 3 < n) {
    s4 = substring(e, i, i+4);
    isself = str_eq(s4, "SELF");
    if (strcmp(isself,"1") == 0) {
    long okL = 1;
    long okR = 1;
    if (i > 0) {
    pc = char_at(e, i - 1);
    if (strcmp(pc,"a") >= 0 && strcmp(pc,"z") <= 0) {
    okL = 0;
    }
    if (strcmp(pc,"A") >= 0 && strcmp(pc,"Z") <= 0) {
    okL = 0;
    }
    if (strcmp(pc,"0") >= 0 && strcmp(pc,"9") <= 0) {
    okL = 0;
    }
    if (strcmp(pc,"_") == 0) {
    okL = 0;
    ...
    if (i + 4 < n) {
    nc = char_at(e, i+4);
    if (strcmp(nc,"a") >= 0 && strcmp(nc,"z") <= 0) {
    okR = 0;
    }
    if (strcmp(nc,"A") >= 0 && strcmp(nc,"Z") <= 0) {
    okR = 0;
    }
    if (strcmp(nc,"0") >= 0 && strcmp(nc,"9") <= 0) {
    okR = 0;
    }
    if (strcmp(nc,"_") == 0) {
    okR = 0;
    ...
    if (okL == 1 && okR == 1) {
    out2 = plant_str_concat(out2, "self");
    i = i+4;
                                      continue;
    }
    }
    }
    out2 = plant_str_concat(out2, c);
    i = i+1;
    }
    return out2;
}
tx_t _handle_cast(tx_t e) {
  ...
  tx_t isid = "";
    ...
    inner = substring(e, estart, eend - 1);
    out = _cat4(out, "plant_math_eval_to_str(\"", inner, "\")");
    pos = eend;
    } else {
    long id_start = pos;
    long id_end = pos;
    while (id_end < elen) {
    ich = char_at(e, id_end);
    isid = find_any(ich, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_");
    if (isid == - 1) {
                                              break;
    }
    id_end = id_end+1;
    ...
    if (id_end > id_start) {
    inner = substring(e, id_start, id_end);
    out = _cat4(out, "plant_math_eval_to_str(\"", inner, "\")");
    pos = id_end;
    } else {
    out = _cat(out, ch);
...
tx_t translate_expr(tx_t expr, PlantArray* nums, PlantArray* evars, PlantArray* maths) {
  ...
  tx_t args1 = "";
    ...
    e = _handle_func_paren3(e, "MATH_SERIES", "plant_math_series_str");
    e = _handle_func_paren4(e, "MATH_TAYLOR", "plant_math_taylor_str");
    ...
    e = _handle_func_paren(e, "MATH_DIVERGENCE", "plant_math_divergence_str");
    e = _handle_func_paren(e, "MATH_CURL", "plant_math_curl_str");
    e = _handle_func_paren(e, "MATH_LAPLACIAN", "plant_math_laplacian_str");
    e = _handle_func_paren3(e, "MATH_LAPLACE", "plant_math_laplace_str");
    e = _handle_func_paren3(e, "MATH_INVERSE_LAPLACE", "plant_math_inverselaplace_str");
    e = _handle_func_paren4(e, "MATH_SOLVE_PDE", "plant_math_solve_pde_str");
    e = _handle_func_paren5(e, "MATH_VERIFY_PDE", "plant_math_verify_pde_str");
    e = _handle_func_paren5(e, "MATH_VERIFY_PDE_STRICT", "plant_math_verify_pde_strict_str");
    ...
    e = _handle_func_paren2(e, "MATH_COMPLEX_MUL", "plant_complex_mul_str");
    e = _handle_func_paren2(e, "MATH_COMPLEX_DIV", "plant_complex_div_str");
    e = _handle_func_paren(e, "MATH_COMPLEX_CONJ", "plant_complex_conj_str");
    e = _handle_func_paren(e, "MATH_COMPLEX_ABS", "plant_complex_abs_str");
    e = _ni_replace(e);
    if (strcmp(e,"NOW") == 0) {
    return "plant_now(\"\")";
    }
    ...
    e = _math_func_paren(e, "ATANH", "math_atanh");
    e = _math_func_paren(e, "LGAMMA", "math_lgamma");
    ...
    e = _math_func_paren(e, "MAT_ADD", "plant_mat_add");
    ...
    e = _handle_func_paren(e, "Result_Ok", "plant_result_ok");
    e = _handle_func_paren(e, "Result_Err", "plant_result_err");
    e = _handle_range(e);
    ...
    e = _math_func_paren(e, "SINH", "math_sinh");
    e = _math_func_paren(e, "COSH", "math_cosh");
    e = _math_func_paren(e, "TANH", "math_tanh");
    e = _math_func_paren(e, "EXP", "math_exp");
    e = _math_func_paren(e, "EXPM1", "math_expm1");
    e = _math_func_paren(e, "LOG10", "math_log10");
    e = _math_func_paren(e, "LOG2", "math_log2");
    e = _math_func_paren(e, "LOG1P", "math_log1p");
    e = _math_func_paren(e, "HYPOT", "math_hypot");
    e = _math_func_paren(e, "SEC", "math_sec");
    e = _math_func_paren(e, "CSC", "math_csc");
    e = _math_func_paren(e, "ERF", "math_erf");
    e = _math_func_paren(e, "ERFC", "math_erfc");
    ...
    e = handle_brackets(e);
    e = _handle_cast(e);
    if (( plant_array_length(maths) ) > 0) {
    e = _wrap_math_vars(e, maths);
    }
    return e;
}
tx_t indent_str(long level) {
...
tx_t gen_create_stmt(tx_t node, PlantArray* subst, PlantArray* nums, PlantArray* evars, tx_t actx, tx_t isel) {
  ...
  tx_t iut3 = "";
    ...
    _aby = find_any(vtype, "[");
    if (_aby >= 0) {
    _ab = substring(vtype, 0, _aby);
    _abt = trim(_ab);
    if (strcmp(_abt,"ARRAY") == 0) {
    _abct = plant_ctype(vtype);
    _inner = substring(vtype, _aby, strlen( vtype ));
    _ci = find_any(_inner, ",");
    _ri = find_any(_inner, "]");
    _size_s = substring(_inner, _ci+1, _ri);
    _size = trim(_size_s);
    tx_t _cv2 = _cat ( "" , cval );
    _plmk = substring(_cv2, 0, 16);
    if (strcmp(_plmk,"plant_list_make(") == 0) {
    _fc = find_any(_cv2, ",");
    long _elems_start = _fc+2;
    long _elems_end = strlen( _cv2 ) - 1;
    _elems = substring(_cv2, _elems_start, _elems_end);
    tx_t _out = "";
    long _pi = 0;
    long _plen = strlen( _elems );
    while (_pi < _plen) {
    _ch = char_at(_elems, _pi);
    long _is_from = 0;
    _fl = substring(_elems, _pi, _pi+11);
    if (strcmp(_fl,"_from_long(") == 0) {
    _pi = _pi+11;
    _is_from = 1;
    }
    if (_is_from == 0) {
    _fd = substring(_elems, _pi, _pi+13);
    if (strcmp(_fd,"_from_double(") == 0) {
    _pi = _pi+13;
    _is_from = 1;
    }
    }
    ...
    if (strcmp(actx,"a") == 0) {
    return _cat3(_cat4(isel, "  ", target, " = "), _cv2, ";\n");
    }
    return _cat4(_cat4(_cat4(isel, "  ", _abct, " "), target, "[", _size), "] = ", _cv2, ";\n");
    }
    }
    if (strcmp(actx,"a") == 0) {
    ...
    return _cat(_cat4(_cat4(isel, "  union ", vtype, " "), target, " = ", cval), ";\n");
    }
...
tx_t gen_if_stmt(tx_t node, PlantArray* nums, PlantArray* evars, tx_t isel, long indent_num, PlantArray* env) {
  ...
  tx_t ebod = "";
    ...
    mexit = env_mexit(env);
    wexit = env_wexit(env);
    env_set(env, 11, _from_long ( indent_num + 2 ));
    bcode = generate_body(bd, env);
    env_set(env, 11, _from_long ( indent_num ));
...
tx_t gen_reap_stmt(tx_t node, PlantArray* sigs, PlantArray* subst, PlantArray* clmap, PlantArray* nums, PlantArray* stvars, PlantArray* evars, tx_t isel) {
  ...
  tx_t rtg2 = "";
    ...
    if (strcmp(arg0,"\"") != 0) {
    aexpr = translate_expr(arg_el, nums, evars, plant_list_make ( 0 ));
...
tx_t gen_cycle_stmt(tx_t node, PlantArray* nums, PlantArray* evars, tx_t isel, long indent_num, PlantArray* env) {
  ...
  tx_t clist = "";
    ...
    cfrom = translate_expr(fromExpr, nums, evars, plant_list_make ( 0 ));
    cto = translate_expr(toExpr, nums, evars, plant_list_make ( 0 ));
    tx_t stepstr = "1";
    if (strcmp(stepExpr,"") > 0 && strcmp(stepExpr,"null") != 0) {
    cstep = translate_expr(stepExpr, nums, evars, plant_list_make ( 0 ));
    stepstr = cstep;
    }
    ...
    clist = translate_expr(listExpr, nums, evars, plant_list_make ( 0 ));
...
tx_t gen_weather_stmt(tx_t node, tx_t isel, long indent_num, PlantArray* env) {
  ...
  tx_t hcode = "";
    ...
    tx_t wnm = _cat("__w", _from_long ( indent_num ));
    wexit = env_wexit(env);
    mexit = env_mexit(env);
    sigs = env_sigs(env);
    subst = env_subst(env);
    ...
    env_set(env, 11, _from_long ( indent_num + 1 ));
    env_set(env, 9, mx0);
    env_set(env, 10, wx0);
    cbcode = generate_body(cb, env);
    env_set(env, 11, _from_long ( indent_num ));
    env_set(env, 9, mexit);
    env_set(env, 10, wexit);
    ...
    env_set(env, 11, _from_long ( indent_num + 3 ));
    env_set(env, 9, mx2);
    env_set(env, 10, wx);
    hcode = generate_body(wbd2, env);
    env_set(env, 11, _from_long ( indent_num ));
    env_set(env, 9, mexit);
    env_set(env, 10, wexit);
...
tx_t gen_throw_stmt(tx_t node, PlantArray* nums, PlantArray* evars, tx_t isel) {
  ...
  tx_t wmsg2 = "";
    ...
    tx_t cmsg = "NULL";
    if (strcmp(wmsg2,"") > 0) {
    cmsg = translate_expr(wmsg2, nums, evars, plant_list_make ( 0 ));
...
tx_t generate_node(tx_t node, PlantArray* env) {
  ...
  tx_t ename = "";
    ...
    indent_num = _to_long(env_indent_num ( env ));
    sigs = env_sigs(env);
    subst = env_subst(env);
    clmap = env_clmap(env);
    actx = env_actx(env);
    ...
    if (strcmp(maths,"") == 0) {
    maths = plant_list_make ( 0 );
    }
    if (strcmp(ntype,"show_stmt") == 0) {
    code = gen_show_stmt(node, nums, evars, isel0);
    return code;
    }
    if (strcmp(ntype,"verify_stmt") == 0) {
    lbl = _map_get(node, "label");
    cond = _map_get(node, "cond");
    lbl = translate_expr(lbl, nums, evars, maths);
    lbl = _handle_cat(lbl, nums, evars);
    ccond = translate_expr(cond, nums, evars, maths);
    ccond = _handle_cat(ccond, nums, evars);
    isn = expr_is_numeric(ccond, nums);
    if (isn == 1) {
    ...
    tx_t body_code = "";
    long bi = 0;
    while (bi < plant_array_length(body_nodes)) {
    bnode = plant_list_get(body_nodes, bi);
    bnode_code = generate_node(bnode, env);
    body_code = _cat(body_code, bnode_code);
    bi = bi+1;
    }
    isel = indent_str(indent_num);
    tx_t has_st = "0";
    long bdi = 0;
    while (bdi < plant_array_length(body_nodes)) {
    bsn = _map_get(plant_list_get ( body_nodes , bdi ), "type");
    if (strcmp(bsn,"setup_stmt") == 0) {
    has_st = "1";
    }
    bdi = bdi+1;
    }
    tx_t has_verify = "0";
    long vdi = 0;
    while (vdi < plant_array_length(body_nodes)) {
    vsn = _map_get(plant_list_get ( body_nodes , vdi ), "type");
    if (strcmp(vsn,"verify_stmt") == 0) {
    has_verify = "1";
    }
    vdi = vdi+1;
    }
    ...
    if (strcmp(ntype,"setup_stmt") == 0) {
    val = _map_get(node, "value");
    cval = translate_expr(val, nums, evars, maths);
    cval = _handle_cat(cval, nums, evars);
    isn = expr_is_numeric(cval, nums);
    if (isn == 1) {
    cval = _cat3("_from_long(", cval, ")");
    }
    isel = indent_str(indent_num);
    return _cat4(isel, "  plant_suite_setup_hook(", cval, ");\n");
    }
    if (strcmp(ntype,"teardown_stmt") == 0) {
    val = _map_get(node, "value");
    cval = translate_expr(val, nums, evars, maths);
    cval = _handle_cat(cval, nums, evars);
    isn = expr_is_numeric(cval, nums);
    ...
    code = gen_create_stmt(node, subst, nums, evars, actx, isel0);
    return code;
    }
    if (strcmp(ntype,"set_stmt") == 0) {
    code = gen_set_stmt(node, nums, evars, isel0);
    return code;
    }
    ...
    if (strcmp(ntype,"bloom_stmt") == 0) {
    bsp = _map_get(node, "species");
    bvar = _map_get(node, "var");
    bisel = indent_str(indent_num);
    tx_t bb9 = "";
    bb9 = _cat4(_cat4(bb9, bisel, "  tx_t ", bvar), " = plant_species_create_by_name(\"", bsp, "\");\n");
    return bb9;
    }
    if (strcmp(ntype,"let_destructure") == 0) {
    dsrc = _map_get(node, "source");
    ditems = _map_get(node, "vars");
    dkind = _map_get(node, "pattern");
    srcc = translate_expr(dsrc, nums, evars, maths);
    isel = indent_str(indent_num);
    tx_t tag = _from_long ( plant_unique_seq ( ) );
    tx_t dsbase = "(";
    dsbase = _cat(dsbase, srcc);
    dsbase = _cat(dsbase, ")");
    tx_t dcode = "";
    dcode = _emit_ds(ditems, dkind, dsbase, isel, tag);
    return dcode;
    }
    ...
    cval = translate_expr(val, nums, evars, maths);
    cval = _handle_cat(cval, nums, evars);
    _aby2 = find_any(vtype, "[");
    if (_aby2 >= 0) {
    _ab2 = substring(vtype, 0, _aby2);
    _abt2 = trim(_ab2);
    if (strcmp(_abt2,"ARRAY") == 0) {
    _abct2 = plant_ctype(vtype);
    _inner2 = substring(vtype, _aby2, strlen( vtype ));
    _ci2 = find_any(_inner2, ",");
    _ri2 = find_any(_inner2, "]");
    _size_s2 = substring(_inner2, _ci2+1, _ri2);
    _size2 = trim(_size_s2);
    tx_t _cv3 = _cat ( "" , cval );
    _plmk2 = substring(_cv3, 0, 16);
    if (strcmp(_plmk2,"plant_list_make(") == 0) {
    _fc2 = find_any(_cv3, ",");
    long _elems_start2 = _fc2+2;
    long _elems_end2 = strlen( _cv3 ) - 1;
    _elems2 = substring(_cv3, _elems_start2, _elems_end2);
    tx_t _out2 = "";
    long _pi2 = 0;
    long _plen2 = strlen( _elems2 );
    while (_pi2 < _plen2) {
    _ch2 = char_at(_elems2, _pi2);
    long _is_from2 = 0;
    _fl2 = substring(_elems2, _pi2, _pi2+11);
    if (strcmp(_fl2,"_from_long(") == 0) {
    _pi2 = _pi2+11;
    _is_from2 = 1;
    }
    if (_is_from2 == 0) {
    _fd2 = substring(_elems2, _pi2, _pi2+13);
    if (strcmp(_fd2,"_from_double(") == 0) {
    _pi2 = _pi2+13;
    _is_from2 = 1;
    }
    }
    ...
    if (strcmp(actx,"a") == 0) {
    return _cat3(_cat4(isel, "  ", target, " = "), _cv3, ";\n");
    }
    return _cat4(_cat4(_cat4(isel, "  ", _abct2, " "), target, "[", _size2), "] = ", _cv3, ";\n");
    }
    }
    if (strcmp(actx,"a") == 0) {
    return _cat3(_cat4(isel, "  ", target, " = "), cval, ";\n");
    ...
    code = gen_give_stmt(node, nums, evars, rty, actx, mexit, isel0);
    return code;
    }
    ...
    target = _map_get(node, "target");
    citem = translate_expr(item, nums, evars, maths);
    ...
    target = _map_get(node, "target");
    ckey = translate_expr(key, nums, evars, maths);
    ckey = _handle_cat(ckey, nums, evars);
    cval = translate_expr(value, nums, evars, maths);
    cval = _handle_cat(cval, nums, evars);
    isel = indent_str(indent_num);
    ...
    isel = indent_str(indent_num);
    hcurl = translate_expr(hurl, nums, evars, maths);
    tx_t hmethod = hmeth;
    if (strcmp(hmethod,"") > 0) {
    ...
    if (strcmp(hy0,"\"") != 0) {
    hy1 = translate_expr(hcbody, nums, evars, maths);
    ...
    if (strcmp(hz0,"\"") != 0) {
    hz1 = translate_expr(htmo, nums, evars, maths);
    htmo = hz1;
    }
    }
    if (strcmp(htmo,"") == 0) {
    ...
    if (strcmp(lsp0,"\"") != 0) {
    lsp1 = translate_expr(lsp, nums, evars, maths);
    lsp = lsp1;
    }
    }
    tx_t lst = ls_tmo;
    ...
    if (strcmp(lst0,"\"") != 0) {
    lst1 = translate_expr(lst, nums, evars, maths);
    lst = lst1;
    }
    }
    if (strcmp(lst,"") == 0) {
    ...
    if (strcmp(rsb0,"\"") != 0) {
    rsb1 = translate_expr(rsb, nums, evars, maths);
    ...
    tgt = _map_get(node, "target");
    cval = translate_expr(tgt, nums, evars, maths);
    cval = _handle_cat(cval, nums, evars);
    isn2 = expr_is_numeric(cval, nums);
    if (isn2 == 1) {
    cval = _cat3("_from_long(", cval, ")");
    ...
    tgt2 = _map_get(node, "target");
    cval2 = translate_expr(tgt2, nums, evars, maths);
    cval2 = _handle_cat(cval2, nums, evars);
    ...
    avp = translate_expr(ap, nums, evars, maths);
    avc = translate_expr(ac, nums, evars, maths);
    ...
    tx_t wsleep = "0";
    if (strcmp(wms,"") > 0) {
    wc2 = translate_expr(wms, nums, evars, maths);
    wsleep = wc2;
    }
    ...
    lcv2 = translate_expr(lv2, nums, evars, maths);
    ...
    code = gen_if_stmt(node, nums, evars, isel0, indent_num, env);
    return code;
    }
    if (strcmp(ntype,"season_stmt") == 0) {
    code = gen_season_stmt(node, nums, evars, isel0, indent_num, env);
    return code;
    }
    if (strcmp(ntype,"cycle_stmt") == 0) {
    code = gen_cycle_stmt(node, nums, evars, isel0, indent_num, env);
    return code;
    }
    if (strcmp(ntype,"weather_stmt") == 0) {
    code = gen_weather_stmt(node, isel0, indent_num, env);
    return code;
    }
    if (strcmp(ntype,"throw_stmt") == 0) {
    code = gen_throw_stmt(node, nums, evars, isel0);
    return code;
    }
    if (strcmp(ntype,"stop_if_stmt") == 0) {
    scond = _map_get(node, "cond");
    scc = translate_expr(scond, nums, evars, maths);
    ...
    csubj = translate_expr(subj, nums, evars, maths);
    isel = indent_str(indent_num);
    ...
    env_set(env, 11, _from_long ( indent_num + 4 ));
    bcode = generate_body(cbody, env);
    env_set(env, 11, _from_long ( indent_num ));
    ccode = _cat(ccode, bcode);
    ...
    tx_t amode_exit = "";
    if (strcmp(mmode,"FAST") == 0) {
    amode_exit = "  plant_fast_reset();\n  plant_fast_exit();\n";
    }
    if (strcmp(mmode,"SAFE") == 0) {
    amode_exit = "  plant_safe_exit();\n";
    }
    if (strcmp(mmode,"SMART") == 0) {
    amode_exit = _cat3("  plant_smart_exit(\"", aname, "\");\n");
    }
    if (strcmp(mmode,"PERSISTENT") == 0) {
    amode_exit = "  plant_persist_exit();\n";
    }
    PlantArray* maths_a = collect_maths ( bd , params , subst );
    bcode = generate_body(bd, env_make ( 1 , sigs , subst , clmap , actx , nums_a , stvars_a , evars_a , rty_a , amode_exit , "" , maths_a ));
    int has_verify = 0;
    long vbdi = 0;
    while (vbdi < plant_array_length(bd)) {
    vbnd = plant_list_get(bd, vbdi);
    vbnd_ty = _map_get(vbnd, "type");
    if (strcmp(vbnd_ty,"verify_stmt") == 0) {
    has_verify = 1;
    }
    vbdi = vbdi+1;
    }
    ...
    if (strcmp(amode_exit,"") > 0 && strcmp(last_ty2,"give_stmt") != 0) {
    bcode = _cat(bcode, amode_exit);
    }
    }
    if (( plant_array_length(bd) ) == 0) {
    if (strcmp(amode_exit,"") > 0) {
    bcode = _cat(bcode, amode_exit);
    }
    }
...
tx_t plant_ctype(tx_t ptype) {
  ...
  tx_t elem_ct = "";
    ...
    ci = find_any(inner_from, ",");
    elem_name = substring(inner_from, 1, ci);
    elem_name2 = trim(elem_name);
    elem_ct = plant_ctype(elem_name2);
    return elem_ct;
    }
    return "tx_t";
}
...
tx_t is_union_type(tx_t t, PlantArray* reg) {
  ...
  tx_t rn = "";
    base = type_base(t);
    tb = trim(base);
    tx_t lk = _cat("UNION.", tb);
    long ri = 0;
    while (ri + 1 < plant_array_length(reg)) {
    rn = plant_list_get(reg, ri);
    if (strcmp(str_eq ( rn , lk ),"1") == 0) {
    return "1";
    }
    ri = ri+2;
    }
...
tx_t collect_union_names(PlantArray* ast) {
  tx_t un = "";
    ...
    tx_t nd = "";
    tx_t nty = "";
    while (ai < plant_array_length(ast)) {
    nd = plant_list_get(ast, ai);
    nty = _map_get(nd, "type");
    if (strcmp(nty,"union_decl") == 0) {
    un = _map_get(nd, "name");
            res = plant_list_add(res, un);
    }
    ai = ai+1;
    }
    return res;
...
tx_t is_bare_id(tx_t e) {
  tx_t is_at2 = "";
    ...
    while (bi2 < strlen( e )) {
    bc = char_at(e, bi2);
    if (strcmp(bc," ") == 0 || strcmp(bc,"(") == 0 || strcmp(bc,")") == 0 || strcmp(bc,"+") == 0 || strcmp(bc,"-") == 0 || strcmp(bc,"\"") == 0 || strcmp(bc,"[") == 0 || strcmp(bc,"]") == 0 || strcmp(bc,"*") == 0 || strcmp(bc,"&") == 0 || strcmp(bc,"=") == 0 || strcmp(bc,"!") == 0) {
    return "0";
    }
    is_at2 = str_eq(bc, "@");
    if (strcmp(is_at2,"1") == 0) {
    return "0";
    }
    bi2 = bi2+1;
    }
...
tx_t emit_inst(tx_t inst, PlantArray* templates, PlantArray* sigs, PlantArray* reg) {
  ...
  tx_t bcode = "";
    ...
    PlantArray* maths_m = collect_maths ( bd , params , subst );
    bcode = generate_body(bd, env_make ( 1 , sigs , subst , plant_list_make ( 0 ) , "" , nums_m , stvars_m , evars_m , rty_m , "" , "" , maths_m ));
    if (( plant_array_length(bd) ) == 0) {
...
tx_t callees_of(PlantArray* bd) {
  ...
  tx_t wbel11 = "";
    ...
    if (strcmp(cty,"set_stmt") == 0 || strcmp(cty,"create_stmt") == 0 || strcmp(cty,"let_stmt") == 0 || strcmp(cty,"give_stmt") == 0 || strcmp(cty,"show_stmt") == 0 || strcmp(cty,"put_stmt") == 0 || strcmp(cty,"verify_stmt") == 0 || strcmp(cty,"setup_stmt") == 0 || strcmp(cty,"teardown_stmt") == 0 || strcmp(cty,"cancel_stmt") == 0 || strcmp(cty,"trace_stmt") == 0 || strcmp(cty,"config_stmt") == 0) {
...
tx_t _map_replace(PlantArray* m, tx_t k, tx_t v) {
  ...
  tx_t samek = "";
    PlantArray* out = plant_list_make ( 0 );
    long i = 0;
    while (i + 1 < plant_array_length(m)) {
    mk = plant_list_get(m, i);
    mv = plant_list_get(m, i+1);
    samek = str_eq(mk, k);
    if (strcmp(samek,"1") == 0) {
            out = plant_list_add(out, mk);
            out = plant_list_add(out, v);
    }
    if (strcmp(samek,"1") != 0) {
            out = plant_list_add(out, mk);
            out = plant_list_add(out, mv);
    }
    i = i+2;
    }
    return out;
...
tx_t _ta_collect_list(PlantArray* lst, PlantArray* tsub) {
  ...
  tx_t x = "";
    long i = 0;
    while (i < plant_array_length(lst)) {
    tn = plant_list_get(lst, i);
    tt = _map_get(tn, "type");
    if (strcmp(tt,"type_decl") == 0) {
    tal = _map_get(tn, "alias");
    trg = _map_get(tn, "target");
            tsub = plant_list_add(tsub, tal);
            tsub = plant_list_add(tsub, trg);
    }
    if (strcmp(tt,"action_decl") == 0) {
    obd = _map_get(tn, "body");
    x = _ta_collect_list(obd, tsub);
    }
    i = i+1;
    }
    return 0;
...
tx_t _ta_rw_list(PlantArray* lst, PlantArray* tsub) {
  ...
  tx_t nbd = "";
  tx_t an2 = "";
    PlantArray* out2 = plant_list_make ( 0 );
    long i = 0;
    while (i < plant_array_length(lst)) {
    tn = plant_list_get(lst, i);
    tt = _map_get(tn, "type");
    if (strcmp(tt,"create_stmt") == 0 || strcmp(tt,"let_stmt") == 0) {
    vt = _map_get(tn, "var_type");
    if (strcmp(vt,"") > 0) {
    rv = _ts_resolve(tsub, vt);
    chg = str_eq(rv, vt);
    if (strcmp(chg,"1") != 0) {
    nn = _map_replace(tn, "var_type", rv);
    tn = nn;
    }
    }
    }
    if (strcmp(tt,"action_decl") == 0) {
    obd = _map_get(tn, "body");
    nbd = _ta_rw_list(obd, tsub);
    an2 = _map_replace(tn, "body", nbd);
    tn = an2;
    }
        out2 = plant_list_add(out2, tn);
    i = i+1;
    }
    return out2;
}
tx_t _ts_resolve(PlantArray* tsub, tx_t name) {
  ...
  tx_t samean = "";
    ...
    tx_t found = "";
    long i = 0;
    while (i + 1 < plant_array_length(tsub)) {
    an = plant_list_get(tsub, i);
    at = plant_list_get(tsub, i+1);
    samean = str_eq(an, cur);
    if (strcmp(samean,"1") == 0) {
    found = at;
                              break;
    }
    i = i+2;
...
tx_t generate_c(PlantArray* ast) {
  ...
  tx_t ct2 = "";
    ...
    PlantArray* species = plant_list_make ( 0 );
    tx_t sp_init_code = "";
    ...
    PlantArray* tsub = plant_list_make ( 0 );
    tcx = _ta_collect_list(ast, tsub);
    ast = _ta_rw_list(ast, tsub);
    i = 0;
    while (i < plant_array_length(ast)) {
    node_el = plant_list_get(ast, i);
    ntype = _map_get(node_el, "type");
    ...
    ntype = _map_get(node_el, "type");
    if (strcmp(ntype,"enum_decl") == 0) {
    nd_code = generate_node(node_el, env_make ( 0 , sigs , esub , plant_list_make ( 0 ) , "" , plant_list_make ( 0 ) , plant_list_make ( 0 ) , eregs , "" , "" , "" , plant_list_make ( 0 ) ));
    ...
    if (strcmp(sname4,"") == 0) {
    sname4 = _cat("plant_anon_struct_", _from_long ( anon_counter ));
    anon_counter = anon_counter+1;
    }
    scn4 = ffi_struct_cname(sname4);
    PlantArray* eargs = plant_list_make ( 0 );
    PlantArray* resolved_tpl = plant_list_make ( 0 );
                resolved_tpl = plant_list_add(resolved_tpl, "type");
                resolved_tpl = plant_list_add(resolved_tpl, "struct_decl");
                resolved_tpl = plant_list_add(resolved_tpl, "name");
                resolved_tpl = plant_list_add(resolved_tpl, sname4);
                resolved_tpl = plant_list_add(resolved_tpl, "generics");
                resolved_tpl = plant_list_add(resolved_tpl, _map_get ( node_el , "generics" ));
                resolved_tpl = plant_list_add(resolved_tpl, "fields");
                resolved_tpl = plant_list_add(resolved_tpl, _map_get ( node_el , "fields" ));
    stdef = struct_typedef(resolved_tpl, eargs);
    ...
    nd_code = generate_node(node_el2, env_make ( 0 , sigs , esub , cmact , "" , plant_list_make ( 0 ) , plant_list_make ( 0 ) , eregs , "" , "" , "" , plant_list_make ( 0 ) ));
    ...
    stmt_code = _cat(_cat4("int main(int argc, char **argv) {\n  plant_init_cli(argc, argv);\n", sp_init_code, dcode, stmt_code), "  plant_async_drain();\n  return 0;\n}\n");
    }
    tx_t td_code = "";
    i = 0;
    while (i + 1 < plant_array_length(tsub)) {
    tal = plant_list_get(tsub, i);
    trg = plant_list_get(tsub, i+1);
    rv2 = _ts_resolve(tsub, trg);
    ct2 = plant_ctype(rv2);
...
tx_t _cl_emit_fn(PlantArray* cnode, PlantArray* sigs, PlantArray* subst, PlantArray* reg) {
  ...
  tx_t bc3 = "";
    ...
    cx3 = translate_expr(bx, nums_c, evars_c, plant_list_make ( 0 ));
    ...
    bc3 = generate_body(bb, env_make ( 1 , sigs , subst , cm2 , "" , nums_c , stvars_c , evars_c , "" , "" , "" , plant_list_make ( 0 ) ));
    fnc = _cat3(fnc, bc3, "");
    }
    fnc = _cat(fnc, "}\n");
    return fnc;
