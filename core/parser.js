...
class Parser {
  ...
  parseIfStatement(coords) {
    ...
    while (!this.isAtEnd()) {
      ...
      if (this.match(TOKEN.KEYWORD, 'ORIF')) {
        ...
        if (this.match(TOKEN.PUNCT, ',')) {
          this.advance();
          ob = this._collectBodyUntil(coords.depth, ['ORIF','ELSE']);
        ...
        if (this.match(TOKEN.PUNCT, ',')) {
          this.advance();
          eb = this._collectBodyUntil(coords.depth, []);
    ...
    // Consume /IF. closer (left unconsumed by _collectBodyUntil)
    if (this.match(TOKEN.PUNCT, '/')) { this.advance(); this.consume(TOKEN.KEYWORD, 'IF', '"IF" after "/" in /IF.'); if (this.match(TOKEN.PUNCT, '.')) this.advance(); }
  ...
  parseEnumDeclaration(coords) {
    ...
    let autoValue = 0;
    while (!this.isAtEnd() && !this.match(TOKEN.PUNCT, '}')) {
      // Skip DEPTH tokens generated by depth-marker preprocessing
      if (this.match(TOKEN.DEPTH)) { this.advance(); continue; }
  ...
  parseArrayLiteral() {
    ...
    while (!this.isAtEnd() && !this.match(TOKEN.PUNCT, ']')) {
      ...
      // Detect bare map literal: { ... }
      if (this.peek(0).type === TOKEN.PUNCT && this.peek(0).value === '{') {
        elements.push(this.parseMapLiteral());
        if (this.match(TOKEN.PUNCT, ',')) this.advance();
        continue;
      }
      const span = [];
  ...
  parseMapLiteral() {
    ...
    while (!this.isAtEnd() && !this.match(TOKEN.PUNCT, '}')) {
      ...
      this.advance(); // consume :
      // Parse value — handle nested arrays/maps recursively
      let value;
      if (this.current().type === TOKEN.PUNCT && this.current().value === '[') {
        value = this.parseArrayLiteral();
      } else if (this.current().type === TOKEN.PUNCT && this.current().value === '{') {
