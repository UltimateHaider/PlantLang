...
function evalCond(expr,soil){
  ...
  if(m=expr.match(/^(.+?)\s+OR\s+(.+)$/i))return evalCond(m[1],soil)||evalCond(m[2],soil);
  if(m=expr.match(/^(.+?)\s+AND\s+(.+)$/i))return evalCond(m[1],soil)&&evalCond(m[2],soil);
  if(m=expr.match(/^NOT\s+(.+)$/i))return !evalCond(m[1],soil);
  if(m=expr.match(/^ANY\s+(\w+)\s+(GREATER THAN|LESS THAN|IS NOT|IS|>=|<=|>|<)\s+(.+)$/i)){const e=soil.get(m[1]);if(!e||!Array.isArray(e.value))return false;const rv=evalExpr(m[3],soil);return e.value.some(el=>cmpOp(el,m[2].toUpperCase(),rv));}
  if(m=expr.match(/^ALL\s+(\w+)\s+(GREATER THAN|LESS THAN|IS NOT|IS|>=|<=|>|<)\s+(.+)$/i)){const e=soil.get(m[1]);if(!e||!Array.isArray(e.value))return false;const rv=evalExpr(m[3],soil);return e.value.every(el=>cmpOp(el,m[2].toUpperCase(),rv));}
  ...
  if(m=expr.match(/^(.+?)\s+(GREATER THAN|LESS THAN|IS NOT|IS|>=|<=|>|<|==|!=)\s+(.+)$/i))return cmpOp(evalExpr(m[1],soil),m[2].toUpperCase().trim(),evalExpr(m[3],soil));
