...
function icon(type){return type==='ok'?'✓':type==='error'?'✕':type==='warn'?'⚠':'›';}
function col(type){return type==='ok'?C.green:type==='error'?C.red:type==='warn'?C.yellow:type==='muted'?C.gray:C.cyan;}
function printLine(text,type){console.log(`${col(type)}${icon(type)}${C.reset} ${text}`);}
...
function main(){
  ...
  switch(cmd){
    case'run':{const file=args[1];if(!file){console.error(`${C.red}✕ حدد ملف .plnt${C.reset}`);process.exit(1);}runFile(file,{mission,verbose});break;}
    case'repl':startREPL({mission});break;
    case'check':{const file=args[1];if(!file){console.error(`${C.red}✕ حدد ملف .plnt${C.reset}`);process.exit(1);}checkFile(file);break;}
    default:
      if(cmd.endsWith('.plnt')||fs.existsSync(cmd))runFile(cmd,{mission,verbose});
      else{console.error(`${C.red}✕ أمر غير معروف: ${cmd}${C.reset}`);process.exit(1);}
  }
}
main();
