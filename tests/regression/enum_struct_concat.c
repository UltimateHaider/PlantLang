...
tx_t main();
tx_t plant_Closure_0_fn(tx_t env, tx_t q);

tx_t plant_Closure_0_fn(tx_t env, tx_t q) {
  tx_t b = ((plant_Env_0*)env)->b;
  return _cat4("cl=", _from_enum(plant_map_get ( b , "color" ), "RED,GREEN,BLUE"), " q=", q);
}
...
tx_t main() {
  ...
  tx_t r = "";
    ...
    plant_iReport_print(get_report(), _cat4("ball=", _from_enum(plant_map_get ( b , "color" ), "RED,GREEN,BLUE"), ":", _from_long ( plant_map_get ( b , "weight" ) )));
    tx_t f = (tx_t)0;
