...
tx_t plant_main() {
    ...
    plant_iReport_print(get_report(), _cat("lease1=", ffi_arc_lease ( o1 , 20 )));
    plant_iReport_print(get_report(), _cat("lease2=", ffi_arc_lease ( o2 , 20 )));
    ...
    plant_iReport_print(get_report(), _cat("status1=", plant_map_to_string ( ffi_persist_status ( ) )));
    ffi_sleep(60);
    plant_iReport_print(get_report(), _cat("gc=", ffi_arc_gc ( )));
    plant_iReport_print(get_report(), _cat("status2=", plant_map_to_string ( ffi_persist_status ( ) )));
  return plant_main;
}
int main(int argc, char **argv) {
  plant_init_cli(argc, argv);
  plant_async_config("PERSIST_PRESSURE", "85");
