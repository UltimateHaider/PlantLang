...
tx_t main() {
    ...
    plant_iReport_print(get_report(), "outer body");
    {
      PlantWeather __w3 = {0};
      plant_weather_enter(&__w3, 1);
      if (setjmp(__w3.buf) == 0) {
    plant_iReport_print(get_report(), "inner body");
    plant_throw("ZERO_STORM", "inner msg");
      } else {
        ...
        if (plant_storm_match(__et, "ZERO_STORM")) {
          ...
          __w3.handled = 1;
          tx_t err = __ev;
    plant_iReport_print(get_report(), _cat("inner caught: ", err));
          plant_storm_release(__ev);
          plant_weather_shelter_leave(&__w3);
        }
        plant_weather_handling_end(&__w3);
      }
    plant_iReport_print(get_report(), "inner calm");
      plant_calm(&__w3);
    }
    plant_iReport_print(get_report(), "inner survived");
      } else {
        const char* __et = plant_exc_type();
        const char* __em = plant_exc_msg();
        tx_t __ev = plant_exc_val();
        ...
        if (plant_storm_match(__et, "ANY_STORM")) {
          ...
          __w1.handled = 1;
    plant_iReport_print(get_report(), "outer any");
          plant_storm_release(__ev);
    ...
    plant_iReport_print(get_report(), "main continues");
    return 0;
}
